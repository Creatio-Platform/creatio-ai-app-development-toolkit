...
class WorkflowCliTests(unittest.TestCase):
    ...
    def test_check_execution_handoff_accepts_runbook_first_artifacts(self):
        with temp_workflow_root() as workflow_root:
            output_dir = Path(workflow_root) / "output" / "TodoList"
            docs_dir = output_dir / "docs"
            write_file(output_dir / "requirements.md", build_valid_requirements_doc())
            write_file(output_dir / "request-spec.json", json.dumps(build_valid_request_spec()))
            write_file(output_dir / "technical-annex.md", "# Technical Annex\n")
            write_file(output_dir / "plan.md", "# Plan\n")
            write_file(docs_dir / "execution-runbook.md", "# Execution Runbook\n")
            planning_result = run_workflow_cli(
                "write-planning-state",
                "TodoList",
                "tester",
            ...
            self.assertEqual(approval_result.returncode, 0, approval_result.stderr)
            handoff_result = run_workflow_cli("check-execution-handoff", "TodoList", workflow_root=workflow_root)
            self.assertEqual(handoff_result.returncode, 0, handoff_result.stderr)
            self.assertIn("HANDOFF_OK TodoList", handoff_result.stdout)

    def test_check_execution_handoff_requires_execution_runbook(self):
        with temp_workflow_root() as workflow_root:
            output_dir = Path(workflow_root) / "output" / "TodoList"
            write_file(output_dir / "requirements.md", build_valid_requirements_doc())
            write_file(output_dir / "request-spec.json", json.dumps(build_valid_request_spec()))
            ...
            self.assertIn("execution-runbook.md not found", handoff_result.stderr)

    def test_check_execution_handoff_rejects_legacy_result_artifacts(self):
        with temp_workflow_root() as workflow_root:
            output_dir = Path(workflow_root) / "output" / "TodoList"
            docs_dir = output_dir / "docs"
            write_file(output_dir / "requirements.md", build_valid_requirements_doc())
            ...
            write_file(output_dir / "plan.md", "# Plan\n")
            write_file(docs_dir / "execution-runbook.md", "# Execution Runbook\n")
            write_file(output_dir / "mcp-application-result.json", "{}\n")
            planning_result = run_workflow_cli(
                "write-planning-state",
                "TodoList",
                "tester",
            ...
            handoff_result = run_workflow_cli("check-execution-handoff", "TodoList", workflow_root=workflow_root)
            self.assertNotEqual(handoff_result.returncode, 0)
            self.assertIn("runbook mode forbids mcp-application-result.json", handoff_result.stderr)
