...
the `branding` guidance, fetched with `get-guidance` (resolve its argument shape via `get-tool-contract`). This file holds only what clio does not ship: the background
