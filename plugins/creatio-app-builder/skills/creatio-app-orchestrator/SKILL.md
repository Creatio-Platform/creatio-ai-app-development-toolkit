...
This skill ships inside the `creatio-app-builder` plugin. Its **toolkit root** is the plugin
directory two levels up from this file (`../../`): it holds `runtime/` and `rules/`, and this
...
files are not accessible from this session, and do not produce a plan from memory.

## Load Order

1. Read `./references/orchestration-policy.md` for the active orchestration contract (approvals, Business Plan format, routing, support mode, execution UX).
...
4. For environment setup, read `./references/01-environment-setup.md`.
5. For requirements gathering, read `./references/02-requirements-gathering.md`.
6. For post-Gate-R implementation and scaffolding (including the transient section-creation failure playbook), read `./references/03-app-implementation.md`.
7. For executable helper behavior: resident tools (`get-tool-contract` index: `resident=true`) are called natively; every other tool is invoked via `clio-run <command>` regardless of transport. `../../runtime/scripts/mcp_client.py` is an **explicit opt-in escape hatch** for hosts with no native MCP transport — not the default fallback (see "Core Rules" below and `./references/orchestration-policy.md`, "clio MCP availability preflight"). Also read `../../runtime/scripts/workflow_validators.py`.
...
- Keep the visible planning artifact in the BA-style Business Plan format defined by `./references/orchestration-policy.md`.
...
- Use `./references/business-checklist.md`, `../../context/essentials.md`, `../../context/naming-conventions.md`, `../../context/clio-cli-reference.md`, and `../../context/model-discovery-evidence.md` as the canonical repository references.
