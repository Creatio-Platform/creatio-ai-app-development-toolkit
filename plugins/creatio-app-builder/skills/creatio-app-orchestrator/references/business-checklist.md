...
The BA draft must surface these defaults in the `UX Expectations` section using the section-6 labels defined in `plugins/creatio-app-builder/skills/creatio-app-orchestrator/references/02-requirements-gathering.md` (the single source of truth for label names) — at minimum `list columns:` and `list filters:`, the form layout (`form groups:` / `form fields:`), default sort when records are time-based, and, for each related list, its add/edit interaction (`add page:` / `edit page:`, or `add/edit: inline in the list`).
