...
REM Compare raw operator input via DELAYED expansion: `if "%KN_MODE_PICK%"==...` textually substitutes the
REM value before the quote closes, so an embedded " would break out and run the rest as commands. !VAR! is
REM substituted after parsing, so the value is inert data.
...
REM Assign via DELAYED expansion (!KN_PICK!), NOT %KN_PICK%: bare %-expansion textually substitutes the raw
REM value before the quoted string closes, so an embedded " would break out and run the pasted remainder as
REM commands (this fires at parse time, before the :vtoken gate below). !VAR! keeps the value inert; the gate
REM at :kn_branch_ready then rejects anything outside the allow-list.
...
REM Track the outcome so the completion banner does not falsely claim "auto-update DISABLED": if this fails,
REM the next clio launch may overwrite the local build with the released nuget version.
set "AUTOUPDATE_OFF=1"
clio autoupdate --disable
if errorlevel 1 (set "AUTOUPDATE_OFF=0" & echo   [warn] 'clio autoupdate --disable' FAILED -- the next clio launch may OVERWRITE this local build with the released nuget version. Re-run, or disable it manually.)
...
REM lock contention). This is a deliberate workaround for a missing clio CLI -- ideally clio would expose a
REM `set-knowledge-source` command and this appsettings surgery could be dropped. The edit rewrites the
REM source into ONE of two shapes based on the mode from [0/7]:
...
REM the clio repo (flagged as a follow-up). Best-effort still: a rewrite failure skips only the sync, not the rebuild.
if errorlevel 1 (
...
if exist "%GEN_MP_DIR%" (
  ...
  mkdir "%GEN_MP_DIR%" >nul 2>&1
)
if not exist "%MP_MARKER%" (>"%MP_MARKER%" echo generated by build-dev-toolchain.bat -- safe to delete when not rebuilding)
...
if "!AUTOUPDATE_OFF!"=="1" (
  ...
  echo   STAGE A: clio global tool rebuilt ^(version !VER!^); WARNING: auto-update could NOT be disabled --
  echo            the next clio launch may OVERWRITE this local build. Disable it manually: clio autoupdate --disable
)
if /I "%KN_MODE%"=="release" (
