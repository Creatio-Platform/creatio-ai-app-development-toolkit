...
function Show-Usage {
    Write-Error "Usage: workflow_gate.ps1 <command> [args...]`nCommands:`n  plan-approve         <AppName> <planner> <routingMode> <credentialsStatus> <understanding> <confirmation>`n  plan-check           <AppName>`n  requirements-approve <AppName> <approver> <text>`n  requirements-check   <AppName>`n  implementation-check <AppName>"
}
if ($args.Count -lt 1) {
    Show-Usage
    exit 1
...
switch ($command) {
    ...
    "implementation-check" {
        & $env:PYTHON_CMD (Join-Path $scriptDir "workflow_cli.py") "check-implementation-plan-gate" @commandArgs
        exit $LASTEXITCODE
    }
    default {
        Show-Usage
