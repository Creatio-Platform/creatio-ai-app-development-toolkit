...
export function claimsBlock(claims, fence) {
  ...
  return `WHAT THE BUILD AGENTS CLAIMED THIS ROUND — a CLAIM, never evidence. Your job includes checking it against what \`get-page\` actually returns:\n${claims.map(line).join('\n')}\n\nA claimed component the page does not carry, and a component on the page nobody claimed, are BOTH \`discrepancies\`.- \`"yes"\` — you looked at the right surface and it carries what the answer asked for (the columns in the \`DataTable\`, the filter on the lookup, the component named).
...
export function resolutionClaimsLine(rows, fence) {
...
export const pairKey = (unit, id) => `${idKey(unit)}\u0000${idKey(id)}`
...
export const pairParts = (key) => { const i = String(key).indexOf('\u0000'); return { unit: String(key).slice(0, i), id: String(key).slice(i + 1) } }
