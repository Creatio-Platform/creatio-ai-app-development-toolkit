...
import {
  ...
  PREFLIGHT_SCHEMA, RECONCILE_SCHEMA, RECONCILE_STATE_MARKER, RECONCILE_TEXT_CAP, REFS_SCHEMA, VERIFIER_SCHEMA,
...
let resolutionCheckTally = new Map()
  ...
  // THE `roundState` SECTIONS OF THE CARRY, in one place because they are one object in the queue file
  ...
  function roundStateCarryLines(carry, j) {
    ...
    // Without this section the carry held the list and no instruction ever asked for it, so the memory died with
    // the process and every resume re-spent the window on the same unit — which is what seeding it from the folder
    // exists to prevent.
...
// The target-package state, seeded from Reconcile and updated by the app unit the moment the package really
  ...
  function* reconcileAttempt(roundNo, id, label, note, attempt) {
    ...
    if (!faults.length) {
      ...
      // The copied state line is checked LAST, after the stand facts, and grafted onto them here so every consumer
      // keeps reading `state.<field>`. A fault here is about the COPY; the retry carries it in the prompt exactly
      // as a shape fault does.
      const { state: merged, fault } = stateFromAnswer(answer)
      if (merged) {
        // The component sweep needs BOTH halves: the types the plan published (computed, inside the line) and the
        // entries the agent resolved (its own answer). It is the one arrival check that cannot run on the answer
        // alone, so it runs here, on the merged state, and its faults spend an attempt exactly as a shape fault does.
        const sweep = []
...
Return \`written\`, \`files\` (every path you wrote) and \`notes\`.`,
  ...
  function foldResolutionEvidence(claims) {
    ...
    for (const c of resolutionContradictions(claims, lastVerifier?.resolutionChecks)) {
      ...
      // the queue file's `discrepancies` rows, which the engine republishes verbatim -- an agent reproduces
