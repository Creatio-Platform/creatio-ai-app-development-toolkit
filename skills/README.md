...
- **`creatio-theme-orchestrator/`** — independent entrypoint skill for theming and branding requests: collects brand colors, fonts, and a theme name, runs the guided palette conversation through clio's theming guidance (`docs://mcp/guides/theming`) and color tool, then builds and applies the theme. A peer of the app orchestrator, not a stage of the app workflow — theming produces no Business Plan and does not apply Gate P/Gate R.
...
- A **separate standalone entrypoint** is warranted only for an independent workflow with its own trigger and no gate order — `creatio-theme-orchestrator` is one (theming has no Business Plan and no Gate P/Gate R). Sequential workflow stages are not.
