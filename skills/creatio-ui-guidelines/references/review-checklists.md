...
- [ ] Every deadline/due-date/SLA field has a timer beside it showing time left/overdue, in the same block, with the date itself still visible and editable — or, where the target version's catalog has no timer component, the closest native alternative was offered instead of a hand-built process-maintained countdown.
