...
**A composite is SEVERAL components — build every part it names, not only the part you recognize.** A Designer-listed combination assembles multiple components (e.g. Approvals = a module/widget above the profile island PLUS the approval list — two components, not one). Fetch the recipe with `get-component-info composite='<caption>'` and build every part; if a spec enumerates a composite's parts, every part is its own required deliverable, not a single row that closes on partial work. This rule reaches the model and still gets applied to only some of a plan's composites — so treat "did I build all parts?" as an explicit check.

- **Record which recipe you fetched for every `compositeOnly` type.** When `get-component-info` reports a type as `compositeOnly`, name — in your worklog/evidence, not only your own reasoning — the exact `composite='<caption>'` recipe you fetched and every part it named. This is a reviewable claim (a reviewer or a later run compares what you recorded against the recipe); only the Approvals module/list pair is machine-checked today (see the migration engine's `Quality gates` row).
...
- **A dense or overloaded layout is a required fix, or a decision to raise — never "refine if desired."** Reread the rendered page against the composition and grouping rules (balance, spacing, grouping) before calling a page done. A layout that only "technically" places every field is not finished work. Group data into logical tabs, field groups, islands, metrics, and progress bars rather than a long unstructured page.
...
- Do not waste large side-profile space when the object genuinely has only one or two key fields — move them into a group or use a header-above-tabs template instead of a big empty side profile. (This is the opposite case from *Balance the page* below: collapse the side when there is little to show; fill it with more islands when there are many columns but the side is under-filled.)
...
- **This also governs scaffolder-created pages — `create-app-section` (and `create-app`) auto-generate the section's list/form pages on a *default* template, with no template choice exposed.** So when a new section's record needs a non-default template (e.g. the DCM progress-bar page), decide it at planning time and do NOT treat the auto-generated form page as final: create the record page explicitly on the right template (`create-page` with the template from `list-page-templates`) and register it as the section's page, or recreate the scaffolded page on that template (the recreate path below). A scaffolder page is fixed to its template exactly like a hand-authored one — it cannot be re-templated in place.
...
- The timer **complements** the field, never replaces it: keep the date visible (users still need the exact date; a read-only or calculated SLA date is fine), and keep both in the same visual block so cause and effect read together.
...
- **Applies to** lookups to **Contact** or **Account**, and comparable person/organization lookups, that name a business relationship (responsible, primary contact, customer, supplier, owner, candidate) — someone the user needs to *reach*. Pure audit lookups (Created by / Modified by) are out of scope.
...
- **Never restyle components to fix a minor nuance — no scope creep.** Do not change a component's default `appearance`/style across the form (e.g. switch every `crt.Input` to `appearance: "outline"`, or change fonts/borders/colors) to work around a small issue or to satisfy another rule. That makes the whole form deviate from base Creatio — a bigger problem than the nuance it fixes. Keep platform defaults; fix only the specific field if truly needed; if a real styling change is warranted, treat it as a deliberate, scoped, confirmed decision, not a global repaint.
- **Custom CSS is a last resort — native inputs first, then warn and confirm (mandatory).** A visual-styling requirement (color, background, font/typeface, text size, spacing/padding/margin, border, width/height, alignment) is custom CSS the moment you express it as a `styles` object, a `classes`/CSS class, OR a component `extraStyles` hook (e.g. `extraStyles.toggle`, `extraStyles.label` carrying `color`/`fill`/`font-family`) — all three carriers count equally; `extraStyles` is NOT "native" just because it is a component input. The Freedom UI font is Montserrat and colors/typography come from theme tokens, so changing a font-family or hard-coding a color is custom CSS, never a native default. Custom CSS is not covered by Creatio's platform-upgrade compatibility guarantees, so it can break on a future upgrade. Mandatory order: **(1)** exhaust the component's native inputs (`get-component-info`; if a native property achieves the look, use it and don't mention CSS); **(2)** only if none can, tell the user no native option exists, warn about the upgrade-compatibility risk, and get explicit confirmation before writing any `styles`/`classes`/`extraStyles`; **(3)** if the user declines, offer the nearest native alternative or state the exact look is not achievable natively. A "just apply / don't ask" instruction waives the question, NOT the one-line upgrade-risk warning — never apply custom CSS silently.
...
- **Show a clear process explicitly, or as a wizard — don't expose the whole data model at once.** Reveal later fields with business rules as earlier input is filled (progressive disclosure).
...
- **ExpansionPanels never sit side by side — they stack vertically, full width.** A `crt.ExpansionPanel` (grouping inputs or wrapping a detail list) always spans the FULL width of its parent tab/container; panels are placed one under another only. Do NOT put two panels in the same row, in two columns, or inside a 2-column grid — two-column layout applies to *fields inside* a panel, not to the panels themselves. Several groups/details in a tab → stack the panels vertically with a gap.
...
Mechanics behind the container-column and no-gap layout rules above — read this when a page shows empty gaps or drifting fields:

- **Check the container's column count BEFORE placing fields.** The grid column count is a property of each container (`columns` / `columnsCount`). Read the actual count `N` of the target container first, then compute placement relative to `N`: a full-width field is `colSpan: N` at `column: 1`; a two-column row is `column: 1` + `column: N/2 + 1`, each `colSpan: N/2`. Do not hardcode `12`/`6`/`7`, and do not place a field at a `column`/`colSpan` beyond `N` (it wraps and leaves blanks).
- **No empty layout gaps — number rows sequentially inside each container.** `layoutConfig` coordinates are LOCAL to the immediate parent and must restart at 1. Within one container number `row` strictly `1,2,3…` with NO skipped index (a field at `row: 3` while `row: 2` is empty creates a blank gap). Never reuse global/absolute row numbers across groups, never give a field an oversized `rowSpan`, and put each field's `parentName` on its real group container.
...
- **Keep a consistent `labelPosition` across a group — but never restyle a component to achieve it.** Use the SAME label position for all inputs in one group/panel so labels align; prefer a concrete value (`"above"`/`"left"`). Do NOT change a component's `appearance` (e.g. `crt.Input` `legacy`→`outline`) just to make a chosen position render cleanly — keeping the base Creatio look wins over the label position (`"auto"` is acceptable when it already renders consistently). See *Typography → custom CSS* for the no-restyle rule.
...
- **Guide non-obvious fields — do not ship bare fields, do not pad obvious ones.** A field that is not self-explanatory needs a `placeholder` (sample value/format) and/or a `tooltip` (meaning, units, allowed values, or how a read-only/calculated value is filled). Guidance is required where it *adds value*, not blanket on every field.
  - **Placeholders are not mandatory on every field.** Add one to free-entry inputs where an example/format helps (text/number/date/email/phone/web). Skip it for self-evident fields and for controls that don't use a free-text hint — lookups/ComboBox (the dropdown shows the options) and checkboxes (the label says it). An unnecessary placeholder is as bad as a missing helpful one.
  - **Write text that genuinely helps — specific, not filler.** "Enter value" or repeating the label is useless. Placeholder when an example/format makes the field self-evident (a date → `dd.mm.yyyy`); tooltip when there is extra meaning the example can't carry (a computed score → how it is derived); both only when format AND meaning both need explaining.
  - Author all tooltip/placeholder text as **localizable strings** (resource keys), not inline literals. A form where non-obvious inputs lack helpful guidance — or one padded with empty/generic/"e.g."-prefixed hints — is a defect, not a finished page.
...
- **Style dialogs like Creatio mini pages** — predictable title placement, button placement, and visual hierarchy.
