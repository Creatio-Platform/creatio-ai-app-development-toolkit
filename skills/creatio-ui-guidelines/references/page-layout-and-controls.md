...
- **Surface related child records as related lists — with a working add flow.** For every business object whose records belong to this one (the `related lists:` entries in the Business Plan — a 1:M child where this object is the parent), put a related list on the record page so those child records are visible and managed in context; do not silently omit it just because the app is simple. Each related list MUST have a **working** way to add a record — never ship one whose records cannot be created, and never leave a toolbar "New" / "+ Add" that errors with *"There is no page for new record"*. Two valid add affordances:
  - **Inline / editable-grid add** — the native pattern for simple children (few fields), and the right default when the child has **no section of its own**. No separate page is required; the row is created in place with the master link pre-set.
  - **A mini page or record form wired to the related list's "+ Add"** — use when the child is field-rich or has its own section; the add action opens that page with the parent FK prefilled.
  Pick by the child's complexity — do NOT force a separate add page on a simple section-less child (that is exactly what triggers the "no page for new record" error). Lookups are NOT related lists (they are dropdown fields, never a list/page). Build the list and its add wiring via clio (`get-guidance related-list`, plus the **"Expanded list"** composite from `get-component-info composite='Expanded list'`) — don't hand-build it.
