...
- **Surface related child records as related lists — with a working add flow.** For every business object whose records belong to this one (the `related lists:` entries in the Business Plan — a 1:M child where this object is the parent), put a related list on the record page so those child records are visible and managed in context; do not silently omit it just because the app is simple. Each such child object MUST have an add/edit page (a mini page for quick add and/or its record form), and the related list's **"+ Add" action MUST open it** — never ship a related list whose records cannot be created. Lookups are NOT related lists (they are dropdown fields, never a list/page). Build the list and its add wiring via clio (`get-guidance related-list`, plus the **"Expanded list"** composite from `get-component-info composite='Expanded list'`) — don't hand-build it.
