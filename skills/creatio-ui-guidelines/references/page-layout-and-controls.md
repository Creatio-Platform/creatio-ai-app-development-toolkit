...
- **Reference information or tools that accompany a record belong in a contextual side panel, not the record body or a modal.** When a page needs supporting context alongside the main record (e.g. a customer summary while handling a case), use a right-docked panel the user opens from the header and can close — not extra fields on the record, not a blocking modal. Reuse the platform's ready panels (attachments, communication) where they exist instead of rebuilding them.
