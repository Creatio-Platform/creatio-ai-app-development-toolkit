...
- **Some fields are not finished alone — an amount needs metrics, a deadline needs a timer, a Contact/Account needs a profile island.** Placing the field satisfies the data model, not the user: each of these three brings an obligatory companion component onto the same page. Resolve every component/composite with `get-component-info` (never author `crt.*` names from memory) and follow *Field-driven companions* in `./references/page-layout-and-controls.md`:
  - **A money/quantity field the user reasons about, typed OR calculated → analytics on that value.** Applies to amounts, totals, prices, hours and the like the user reasons about — not a bare line-item quantity (e.g. a qty on a mini page). Add at least one *useful* metric about it — rollup across related records, paid vs remaining, plan vs actual, average per unit/period, share of a larger total — as an XS/S indicator/gauge with an icon in the profile island, or a chart in the content area / a dedicated Analytics tab when there are several. A lone money field with no analytics anywhere is an unfinished page, and a "metric" that merely repeats the field's own value is not analytics.
  - **Deadline / due date / SLA date → a timer next to it.** Put a timer component beside the date (same island/group), bound to that column, so the user sees time left / overdue instead of doing date math. Keep the date itself visible — the timer complements the field, it never replaces it. If the target version's catalog has no timer component, say so and offer the closest native alternative rather than hand-building a process-maintained countdown.
  - **A business-relationship Contact / Account lookup → an island with that record's profile.** Applies to a person/organization the user needs to reach (responsible, primary contact, customer, supplier, owner) — not pure audit lookups (Created by / Modified by). Add a read-only related-record profile island — name, job title, company, phone, email, communication options — captioned for the relationship (`Primary contact`, `Customer`), so the user can reach the person without leaving the page. Bind its visibility to the lookup so an unfilled record shows no blank profile.
- **Choose the page template BEFORE creating the page — a staged record needs the progress-bar template, not the default one.** The template is fixed at `create-page` and cannot be swapped afterwards (retrofitting means recreating the page and re-authoring its body), so derive it from the page's intended composition first. If the record is meant to show ordered process state (DCM / stage progress bar), create it from the template that already ships the progress bar instead of hand-placing a bar into the default form page. Template names are version- and environment-specific: resolve them with `list-page-templates` against the target environment and pick from what it returns — never from memory. This includes scaffolder-created section pages — `create-app-section` builds them on a default template, so create and register the record page on the right template up front rather than accepting the auto-generated one.
