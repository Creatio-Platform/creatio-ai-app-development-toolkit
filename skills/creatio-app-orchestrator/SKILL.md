...
- **External-user (portal) access is a first-class planning decision whenever the domain implies it.** This plan is a HANDOFF the implementing agent applies later, so external access has to be DECIDED in the plan, not discovered at build time. "External users" is the CONCEPT — customers, partners, vendors, suppliers, applicants, citizens, or any self-service / client audience — NOT the literal word "portal"; recognise it from the domain, never key off a keyword. Two triggers put it in the plan: (a) PROACTIVELY, as a domain expert — when the app's domain plausibly serves an audience outside the company (a customer/partner/self-service scenario), you MUST ASK the developer, in the FIRST discovery batch, whether part of the app should be exposed to external users, rather than waiting for them to raise it (per `../../context/business-checklist.md`, "Users, access and ownership"); (b) EXPLICITLY — when the request already states that external / portal / self-service access must be provided, include it without asking. When external access is in scope you MUST (1) carry the external audience (the `All external users` role) in `## 2. Roles and Permissions`, and (2) add the CONDITIONAL top-level section **`## 8. Portal section`** (it sits before Edge Cases, which then becomes `## 9. Edge Cases and Exceptions`), structured exactly like `## 6. UX Expectations` — no prose, just the section name and structure: a `portal sections:` count line (how many of the app's sections are exposed to external users, named) and, per exposed section, the external surface using the section-6 labels (`list columns:`, `form groups:` / `form fields:`, `add page:` / `edit page:`) plus an `external access:` level (default **read**; widen to create/edit only where external users genuinely author records). **Keep the external field set DELIBERATELY SMALL — significantly fewer fields than the internal page.** An external surface exposes only what the customer needs; NEVER surface internal-only information to external users (assigned agent/owner, internal notes/comments, costs/margins, internal status or reason codes, other customers' data). Choosing the external `list columns:` and form fields is an act of data minimisation, not a copy of the internal page. The section is CONDITIONAL: OMIT it entirely when external access is out of scope (Edge Cases stays `## 8`), and never add it as empty filler. At implementation time the build steps — bind the page for the portal audience, place the section in an external workplace, grant the object (and its lookups) to `All external users` — are owned by `get-guidance name=portal-sections`; do not improvise them here.
