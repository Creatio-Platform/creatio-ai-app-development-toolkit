...
- **Web→mobile page conversion is a dedicated skill.** Whenever the task is to make an existing Freedom UI web page available in the Creatio Mobile app (convert/port a page to mobile, build a mobile list/form page, or register a converted page as a mobile section/workplace), invoke the **`creatio-mobile-page-conversion`** skill — it loads the conversion playbook and enforces the conversion gates (Gate M before any write, Gate S before any section registration). Do not convert from memory or skip the gates.
