...
- **Run a clio MCP availability preflight before the first clio operation, and fail fast when it is unavailable.** Confirm native clio MCP tools are surfaced (e.g. `get-tool-contract` resolves) before any clio work. If clio MCP is unavailable — the server did not launch, no native tools were surfaced, or a registered environment is unresponsive — STOP and return a **prerequisites blocker** (install .NET, install clio via `dotnet tool install clio -g`, register the environment via `clio reg-web-app`). Do not self-bootstrap: **do not install** or download the .NET SDK, do not change PowerShell `ExecutionPolicy`, and do not silently register environments. Treat a registered-but-unresponsive server as unavailable — one confirmation probe, then the blocker; do not retry indefinitely. Full contract in `../../AGENTS.md`, "clio MCP availability preflight".
