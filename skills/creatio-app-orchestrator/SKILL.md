...
- **Schema naming is mandatory, not optional — and it starts at planning.** You MUST invoke the **`creatio-schema-naming`** skill at whichever of these two points applies, and apply its rules together with `../../context/naming-conventions.md`: (1) **during plan formation**, before any object or column code appears in the Business Plan `## 3. Object Model` (see `../../runbooks/02-requirements-gathering.md`, "Schema Naming"), so the plan carries skill-validated names; and (2) **before creating or renaming** data-model elements (`create-entity-schema`, `update-entity-schema`, `create-lookup`, and the objects/columns implied by `create-app`/`create-app-section`) — including targeted, implementation-ready changes that skip the Business Plan stage entirely, so the skill still runs even when there is no plan. Route every object, title, column, field, lookup, Guid/UId, and relation-object name through the skill **before** choosing it. Do not invent names from memory — inconsistent or non-conventional names are hard to correct after the schema is published.
