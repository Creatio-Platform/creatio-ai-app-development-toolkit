...
- **Emit a second deliverable: `behaviour-index.json`** next to the report — one entry per row,
  ...
  **plain-language columns the migration plan shows the human approver**, so write them for a reader
  ...
  - **`whatItDoes`** — one or two sentences naming what the logic does in the user's terms. Make it
    COMPLETE, not a label: say what STARTS it (the trigger — a field changing, the record saving, the
    page opening), what it reads and what it changes (the fields/data, in the user's words), and any
    edge case that matters (what happens on empty/clear, or when a value can't be read). This is the
    card's *What it is*, restated fully for the row.
