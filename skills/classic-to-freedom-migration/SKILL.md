...
These six rules are non-negotiable. Everything else in this skill serves them. **When prose and an engine gate disagree, the gate wins** — do not re-argue a rule the engine already enforces.
...
2. **The plan you present IS the verbatim output of `node engine/migrate.mjs <manifest> --plan`.** Fill the `<FILL: …>` placeholders and paste the whole thing. Never hand-author, paraphrase, reorder, reformat, or drop a generated section. Every correction or enrichment goes in an **Adjustments** list at the very end — never by editing a generated table. Hand-authoring the plan is *the* recurring failure: it silently drops the List page, child pages, and columns the engine produced.
...
- Produce one consolidated plan: each section a sub-plan, sharing discovery, placement, and validation sections.
...
- `plan.md` holds **the pasted `--plan` output** (Contract rule 2) plus its provenance; it is **frozen after approval** — changes go through `decisions.md` and re-approval.
...
> Arg-facts: `list-schema-hierarchy` returns each schema's **UId**; `get-classic-schema-by-uid` takes **`schema-uid`** (a UId, not a name). Do **NOT** offload the fetch to a general-purpose sub-agent — it just duplicates this context.
...
- `list-schema-hierarchy` + `get-classic-schema-by-uid` to enumerate the full replacing-schema chain (ordered by `HierarchyLevel`) and read every schema body — the input the merge in step 4 consumes.
...
- **`resources`** — the schema's localizable strings (`{ "SomeTabCaption": "Vacancies", … }`, from `SysLocalizableValue`) → tab/group/detail captions resolve to real labels, not `Resources.Strings.*` keys.
...
- **`structure` — input completeness.** `complete` only when the manifest carries the RESOLVED inputs (not FILL-slot promises): `detailSchemas` for every custom detail, and `childPageSchemas` for every child whose detail names a real edit page (`getEditPageName`). **Your first `--plan` run will normally come back STRUCTURE INCOMPLETE** — read `issues[]`, fetch exactly those schemas, add them, re-run until `complete` is true. There is no way to "skip" this in code.
...
**Present the plan = the verbatim `--plan` output (Contract rule 2).** Run `node engine/migrate.mjs <manifest> --plan`, fill every `<FILL: …>` placeholder (scope, environment, package, approach, business sentence, template choices), and paste it whole. The output already carries the user-facing summary (its `Overview` / `What it does` / `Main scope` — formatted per `./references/analysis-summary.md`), the design spec in Main-scope order (`List page` → the form page's `Layout`/`Logic`/`⚠ Confirm`), and the `Child page mappings` section — so it is one artifact, not several. Do not also hand-write a separate summary or a template-shaped plan. Whole-package → run `--plan` per page in the current phase; keep the rest in `plan.md`.
...
4. Subtask order within a page: package/app/page scaffolding → package placement setup → template creation or existing-page selection → entity/data-source adjustments → layout → business rules → **child edit pages, then the parent's related lists** → details/related lists/standard features → handlers/converters/validators → backend/service → localization/bindings → switch-over (only if approved).
   - **Build every native feature UP FRONT as its native component — never build a generic Expanded-list/DataGrid first and "switch" it later.** A Visa = Approvals is `crt.ApprovalList` *because it is an Approval* (it brings its own approve/reject actions, so it needs no child edit page) — "the child has no edit page / it's view-only" does not reclassify a `standardFeatures` entry into a list. Confirm the component on-stand (`get-component-info`) before building the tab.
