...
2. **Merge + map with the bundled engine — do NOT hand-merge when Node is available.** Write a manifest `{ "entity", "entityColumns", "schemas":[{"pkg","body"}], "seed":[{"pkg","body"}], "clientEditableLayers":[…], "resources":{…}, "columnTitles":{…}, "detailSchemas":{…}, "childPageSchemas":{…}, "template", "targetPackage" }` (paste each `get-classic-schema-by-uid` body inline). **Supply the resolution inputs so the spec shows REAL names, not codes** (otherwise the engine flags them for you):
   ...
   - **`childPageSchemas` (REQUIRED — fetch EVERY child edit page before the plan)** — for EACH related list, the child entity's own **edit-page** schema as a NESTED manifest (`{ "<editPage or child entity>": { "entity", "schemas":[…], "seed":[…], … }, … }`, bodies via `get-classic-schema-by-uid`; resolve the edit-page names via `list-pages` by child entity). The engine then **recursively maps each child page and nests its full field-mapping design spec** under `### Child page mappings` — so the detail pages are actually MAPPED, not just listed. A `### Child page mappings` section left as `<FILL: recursive sub-migration>` slots (no field tables) means you skipped this — resolve it BEFORE presenting, not at build time. Only a child whose edit page genuinely does not exist yet legitimately stays a `<FILL>` slot (and you say so).
