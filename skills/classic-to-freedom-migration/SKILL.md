...
2. **Merge + map with the bundled engine — do NOT hand-merge when Node is available.** Write a manifest `{ "entity", "entityColumns", "layers":[{"pkg","body"}], "seed":[{"pkg","body"}] }` (paste each `get-classic-schema` body inline) and run `node engine/migrate.mjs <manifest.json>` (the `engine/` directory bundled beside this SKILL.md; resolve its absolute path in the plugin dir). It returns the effective page + the Freedom **ChangeSet** + `needsDecision[]` (your 20% worklist) + `parseErrors` / `warnings` / `unresolvedParents` diagnostics + `effective.seedQuality` (whether your `seed` is a real fetched template body or a hand skeleton — #19). Re-run it with `--spec` (`node engine/migrate.mjs <manifest.json> --spec`) to get the ready-to-present per-page **design spec** Markdown for the approval gate (steps 5–6).
