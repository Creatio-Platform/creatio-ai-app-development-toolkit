...
**Always present the migration plan to the user before any implementation — for every scope (single-section, whole-package, and existing-Freedom reconcile), with no exceptions.** Never jump from analysis straight to building, and never treat a task as "too small" or "obvious" to skip the plan. At the approval gate present the **whole plan, GENERATED by `node engine/migrate.mjs <manifest> --plan`** — Overview + What it does + Pages (with `<FILL: …>` placeholders for the few agent decisions) + the design spec (`Layout` / `Section` / `Logic` / `⚠ Confirm`) + the recursive `Child pages` table. **Fill every `<FILL: …>` placeholder, then paste the output VERBATIM.** Any correction or enrichment (e.g. a mis-guessed control) goes in an **Adjustments** list at the very END — **never edit, re-format, or drop a generated section/table.** Hand-authoring the plan, or curating the generated spec, is the recurring failure that silently drops the `Section`/`Child pages`/columns the engine produced. Only exhaustive provenance stays in `plan.md`; a gate that shows a paraphrase — not the `--plan` output — is NOT sufficient. For the reconcile scope, the plan must include the reconciliation diff (what will be added, modified, and removed/hidden on the existing Freedom section). Presenting the plan and stopping for approval is a required step, not an optional one.
...
**Present the whole plan by running `node engine/migrate.mjs <manifest> --plan`, filling every `<FILL: …>` placeholder, and pasting the output VERBATIM.** The `--plan` output already contains the Overview/What-it-does/Pages header, the design spec (`Layout`/`Section`/`Logic`/`⚠ Confirm`), and the recursive `Child pages` table — you only supply the placeholder values (scope, environment, package, approach, business sentence, template choices). **Do not hand-author the plan, do not re-format, and do not drop any generated section** (that silently loses the `Section` block, list columns, child pages, etc. — the recurring failure). Corrections/enrichments (e.g. a mis-guessed control, a resolved process name) go in an **Adjustments** list at the very end, never by editing the generated tables. (Whole-package: run `--plan` per page in the current phase; keep the rest in `plan.md`.)
