...
3. Build each page from its per-page design spec (`references/page-design-spec.md`): create or select the page from the spec's template, then add containers, fields, rules, handlers, and detail data sources in the spec's build order. **Migrate RECURSIVELY — the migration is a page TREE, not one page:** for every **`Rebuild (child)`** row in the plan's `Pages` table, run the full pipeline on that child entity's edit page (resolve its schema via `list-pages`, fetch its layers + seed, `migrate.mjs --plan`, build it) and, if the classic section used an add-record mini page, migrate that too. A related list whose child entity has no Freedom form cannot add/edit records, so a parent page whose children were not migrated is NOT done. Execute subtasks in dependency order:
