...
> **Tooling (read this FIRST — it avoids a long, token-burning rediscovery):** the classic-schema readers — `list-schema-hierarchy`, `get-classic-schema-by-uid`, `list-entity-client-schemas`, `get-tool-contract`, `odata-read` — are **hidden long-tail** clio tools. **Invoke them through `clio-run`** (`{ "command": "<tool>", "args": { … } }`). Do **NOT** `ToolSearch` for them — that returns nothing and wastes turns. Two arg facts: `list-schema-hierarchy` returns each schema's **UId**, and `get-classic-schema-by-uid` takes **`schema-uid`** (a UId, not a schema name). You do **NOT** need to read the `engine/` source (`README`, `migrate.mjs`) to learn the manifest — its shape is in step 4.2. And do **NOT** offload the fetch to a general-purpose sub-agent; run the `clio-run` calls inline (a sub-agent just duplicates this context).
...
2. **Merge + map with the bundled engine — do NOT hand-merge when Node is available.** Write a manifest `{ "entity", "entityColumns", "schemas":[{"pkg","body"}], "seed":[{"pkg","body"}], "resources":{…}, "columnTitles":{…}, "detailSchemas":{…} }` (paste each `get-classic-schema-by-uid` body inline). **Supply the three resolution inputs so the spec shows REAL names, not codes** (otherwise the engine flags them for you):
   ...
   - **`detailSchemas`** — each custom detail's own fetched body + its title (`{ "StageInRecruitmentDetailV2": { "body": "<define(...)>", "title": "Stage history" } }`, body via `get-classic-schema-by-uid`, title from the detail's `SysLocalizableValue`) → auto-named details, their related-list columns, and the detail's display title all resolve (#11ii/#13).
   ...
   - **`childPageSchemas`** — for EACH related list, the child entity's own **edit-page** schema as a NESTED manifest (`{ "<editPage or child entity>": { "entity", "schemas":[…], "seed":[…], … }, … }`, bodies via `get-classic-schema-by-uid`). The engine then **recursively maps each child page and nests its full design spec** in the plan under `### Child page mappings` — so the child pages are actually MAPPED, not just listed. Fetch these once you know the child edit pages (resolve names via `list-pages` by child entity); any child you omit gets an explicit `<FILL: recursive sub-migration>` slot instead (still a required deliverable — see step 7).
