...
> Arg-facts: `list-schema-hierarchy` returns each schema's **UId**; `get-classic-schema-by-uid` takes **`schema-uid`** (a UId, not a name); `list-entity-client-schemas` takes **`entity-name`** and returns `sections` + `editPages` (each with `kind: classic|freedom`, per-type `typeColumnValue`, and `miniPageSchema`). Do **NOT** offload the fetch to a general-purpose sub-agent — it just duplicates this context.
