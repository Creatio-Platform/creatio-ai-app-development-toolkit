...
2. **Merge + map with the bundled engine — do NOT hand-merge when Node is available.** Write a manifest `{ "entity", "entityColumns", "layers":[{"pkg","body"}], "seed":[{"pkg","body"}] }` (paste each `get-classic-schema` body inline) and run `node engine/migrate.mjs <manifest.json>` (the `engine/` directory bundled beside this SKILL.md; resolve its absolute path in the plugin dir). It returns the effective page + the Freedom **ChangeSet** + `needsDecision[]` (your 20% worklist) + `parseErrors` / `warnings` / `unresolvedParents` diagnostics.
   **⛔ HARD GATE — do NOT proceed to the plan or the implementation while ANY of these is non-empty:**
   ...
   - ⚠️ An empty `unresolvedParents` obtained by pasting a **hand-written skeleton** as the seed is a FALSE all-clear: the gate is only meaningful when the seed is the real fetched template body (step 4.1). If you clear the gate, confirm the seed came from `get-classic-schema`, not from a skeleton you typed — otherwise base actions (`Run process`) and the true container nesting are still missing.
...
For every page classified as Rebuild or Delta, produce a per-page design spec following `references/page-design-spec.md`: the Classic page walked region by region and mapped onto a Freedom template, with each field's component, data-source attribute, and grid placement, its page rules and handlers, a wireframe of the finished page, and a build order. **Build it from the engine ChangeSet, not by hand:** every `viewConfigDiff` insert is a field row (its `parentName` = the target container/tab, its `layoutConfig` = the grid placement + colSpan), every `details` / `standardFeatures` entry is a related-list / native-feature row (with its target tab), every `cardActions` entry is an action row, and every `needsDecision` item is accounted for in the row it affects. This spec is the build contract for step 7; it is required before the plan is presented (step 6); attach it to the page's sub-plan in `plan.md`.
