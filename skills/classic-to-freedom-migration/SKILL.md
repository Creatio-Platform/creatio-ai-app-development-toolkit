...
- **DCM widgets mis-placed / mis-built.** The **progress bar** (`crt.EntityStageProgressBar`) goes INSIDE `MainHeader`, not as a bare child of `Main`. **Next steps** is a tab BESIDE Feed/Attachments, built like them (caption via `#ResourceString#`, icon, header in the tab's `tools` slot, widget in `items`) — not a bare widget and not an ExpansionPanel. Both auto-populate from the case; don't hand-author stages/steps. → the mapping reference's build recipes.
- **Run-process button mis-placed.** Read the process BINDING (`ProcessInModules` by SysModule): a list/registry-bound process launches from the **List page's existing `Actions` button as a menu item** (not a standalone button, not on the form), labelled with the process **Caption** (not its code). → the mapping reference's build recipes.
