...
- **`typedPages` + `typedPageSchemas` (required for a typed entity — each per-type form is a FULL deliverable, folded at PLAN time, gated)** — for a **typed entity** (records open a DIFFERENT edit page per Type — e.g. `Document` → `DocumentICPage`/`DocumentOCPage`/`DocumentRegistryPage`/`ActPageV2`), the per-type Classic edit pages ARE the form deliverables (the base `*PageV2` is only their shared parent/seed, **not** a separate form). Get the list from `list-entity-client-schemas` (`editPages` with per-type `typeColumnValue`) → `manifest.typedPages` (`[{ schema, type }]`). Then **fold EACH**: assemble its own bundle (`get-classic-migration-bundle --schema-name <TypedPage>`) into `manifest.typedPageSchemas["<TypedPage>"]` (a nested manifest, exactly like `childPageSchemas`) so the engine emits its FULL per-type design spec under `### Typed page mappings`. The ONLY escape is `{ "bindOnly": true }` on the entry — use it strictly when that type's layout is **identical to the base** (a type-specific binding, no separate form). **The structure gate BLOCKS the plan until every typed page is folded or `bindOnly`** — "per-type field mapping done at build" is not a valid resolution (it is exactly what shipped 1 of 4 typed forms). Fetching all N typed bodies is inherent: you cannot build a form whose body you never read.
