...
- **`section`** — the `*Section` chain → the engine reports the LIST page's add-record mini page, **quick filters** (the registry fixed-filter bar — `initFixedFiltersConfig`, e.g. a period/date + owner filter), **section actions** (custom `getSectionActions` menu items, incl. the standard `getButtonMenuItem`/`Click.bindTo` shape), and list columns in the spec's `### List page` block (a surface the record-page migration does not cover). The bundle already gathers the section body, so these auto-extract — but they still MUST be built: rebuild the quick filters as the Freedom list's filter controls and the section actions as list-page actions; do NOT ship a list page with the registry filter bar or its custom actions dropped.
- **`signals` (required — resolve the ⚠ on-stand checks at PLAN time, not at build)** — the plan is **INCOMPLETE** (⛔, non-zero `--plan` exit) until you record the three conditional-feature checks in `manifest.signals`, each `{ "resolved": true, "present": <bool>, "cases"|"items"|"names": [...] }`: **`dcm`** (`SysSchema WHERE ManagerName='DcmSchemaManager'` for the entity/family → progress bar + Next steps), **`processes`** (`ProcessInModules` by the section `SysModule`, names via `VwSysProcess` → Run process), **`printables`** (`SysModuleReport` by `SysModule`, `ShowInSection`/`ShowInCard` → Print). Run them with the existing `execute-esq`/`odata-read` tools — there is no dedicated command and none is needed. **`present:false` (checked on-stand, none found) is a VALID resolved answer**; only a missing/unresolved key blocks — the distinction is "verified none" vs "never checked". This gate is what ends the recurring "build faithful to the classic body, check the case/process/print later" miss: the answers are now in the plan the user approves, and the engine renders them under `### On-stand signals`.
- **`typedPages`** — for a **typed entity** (one whose records open a DIFFERENT edit page per Type — e.g. `Document` → `DocumentICPage`/`DocumentOCPage`/`DocumentRegistryPage`/`ActPageV2`), the per-type Classic edit pages. Get them from `list-entity-client-schemas` (its `editPages` with per-type `typeColumnValue`) and supply as `manifest.typedPages` (array of `{ schema, type }`, or bare schema names). The engine then lists EACH in Main scope and raises a `typed-page` ⚠: the Classic per-type pages take PRECEDENCE over a general Freedom RelatedPage binding, so a single Freedom form leaves "+ New"/open-record routing to Classic for every other Type. Plan a per-type form (rebuild or a type-specific binding by the Type column) for EACH type — never collapse the family to one form. Omitting `typedPages` silently drops the whole per-type family from the plan.
...
- **Typed-entity per-type pages collapsed to one form.** A typed entity opens a different Classic edit page per Type (`DocumentICPage`/`OCPage`/…). Those Classic pages take PRECEDENCE over a general Freedom binding, so one Freedom form leaves every other Type routing to Classic. Run `list-entity-client-schemas`, supply `manifest.typedPages`, and plan a per-type form/binding for EACH type — the engine lists them in Main scope + raises a `typed-page` ⚠. Do not fold the family to one form (nor discover the precedence only at build). → step 4.2 (`typedPages`).
