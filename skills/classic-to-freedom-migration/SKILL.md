...
**Build each standard feature in its correct shape — read the `uiShape` (#3/#6).** Every `standardFeatures` entry carries a `uiShape` that says HOW it renders; the two shapes are NOT interchangeable, and neither is a generic Expanded-list you invented:
- **`uiShape: "component"` (Approvals, Attachments, Feed)** → a distinct native Freedom component. Build it as that component, **never** as a generic list/DataGrid. In particular a **Visa = Approvals** (`crt.ApprovalList`): its records live in a `*Visa` entity (e.g. `ApplicantVisa`) with an FK to the master record, and that is *exactly* how Creatio stores Approvals — so "it's just a related list over ApplicantVisa filtered by the master" is NOT a reason to reclassify `VisaDetailV2` as a plain list (a real run made this mistake).
- **`uiShape: "list"` (Activities, Emails)** → these ARE filtered related lists: a DataGrid of the child records (Activity/Task, Email) filtered to the master record, the same UI as any other child list. Build them as a **related list** — that is their native form, not a downgrade. Do **NOT** turn them into a **Timeline** (the aggregate chronological feed) or an email-client component (a real agent rebuilt Activities/Emails as a `crt.Timeline` — wrong). A Timeline is migrated ONLY when the classic page has an actual Timeline component (the engine reports that under `widgets`, not `standardFeatures`).

Only entries in `details[]` are the ordinary (non-feature) related lists. Keep an engine-matched standard feature AS its shape unless you confirm on-stand it genuinely does not use that feature's infrastructure.
...
3. Build each page from its per-page design spec (`./references/page-design-spec.md`): create or select the page from the spec's template, then add containers, fields, rules, handlers, and detail data sources in the spec's build order. **Migrate RECURSIVELY — the migration is a page TREE, not one page.** Each **`Rebuild (child)`** row in the plan's `Main scope` table has its OWN design spec under the plan's **`### Child page mappings`** section — build each child page from that nested spec exactly as you build the parent. That mapping is produced at PLAN time by passing the child edit-page schemas as `childPageSchemas` (step 4.2), so it is already in the approved plan; **a `<FILL: recursive sub-migration>` slot must have been resolved BEFORE approval** (fetch the child's edit page via `list-pages` → add it to `childPageSchemas` → re-run `--plan`), never deferred to build. If the classic section used an add-record mini page, migrate that too. A related list whose child entity has no Freedom form cannot add/edit records, so a parent page whose children were not migrated is NOT done. **You do not get to declare a child "out of scope" — there is no out-of-scope here; a child with a real Classic `*Page` is migrated regardless of its size, and any scope trim is the user's call.** Execute subtasks in dependency order:
   ...
   - details, related lists, and standard features — build **every** `details[]` entry from the ChangeSet as a Freedom related list, and **every** `standardFeatures[]` entry as its native Freedom feature (Approvals / Attachments / Activities (Tasks) / Emails), each in the tab the design spec assigns. **Build the native feature UP FRONT — do NOT build a generic Expanded-list/DataGrid first and "switch" it to the native component later** (a real run built the Visa tab as a plain `ApplicantVisa` list, then replaced it with `crt.ApprovalList` — the first build was wasted and shippable-if-unnoticed). **The "child has no Freedom edit page / the classic detail is view- or attach-only" reasoning does NOT reclassify a `standardFeatures` entry into a list** — a Visa=Approvals is `crt.ApprovalList` (which brings its own approve/reject actions, so it needs no child edit page) *because it is an Approval*, not a list that happens to lack an add button. Confirm the native component exists on-stand (`get-component-info`) BEFORE building the tab, then build it as that component the first time. Resolve any `detail-unresolved` item (auto-generated `SchemaNDetail` name) by fetching the detail schema FIRST, and confirm each entity-inferred Attachments before relying on it. For every `detail-editpage` flag, resolve the child entity's **Freedom edit page** (and its **mini page**, if the classic detail used one) — the form that opens when a related-list row is added/edited is a SEPARATE migration, not covered by migrating this master page: confirm a Freedom form already exists for that child entity, or migrate it as a follow-on page (a related list whose entity has no Freedom form cannot add/edit records). **Nothing here may be silently skipped:** an entry you cannot build now is logged as a loud `TODO` / `BLOCKED` in `worklog.md` with the reason — a page that migrated fields and rules but dropped its details/features (or their edit pages) is NOT done.
