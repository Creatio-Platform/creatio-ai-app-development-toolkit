...
const partFile = (outDir, label, round = 1, index = 0) => {
...
2. \`indexEntries\` — one entry per key listed above that you covered, keyed EXACTLY as written above, naming the card and the AC numbers. In each entry ALSO write two plain-language fields the migration plan shows the human approver — write them for a reader with NO technical knowledge (no method names, no framework terms): \`whatItDoes\` — one or two sentences on what the logic does in the user's terms; make it COMPLETE, not a label: name what STARTS it (the trigger — a field changing, the record saving, the page opening), what it reads and what it changes (the fields/data, in the user's words), and any edge case that matters (what happens on empty/clear, or when a value can't be read); and \`useCase\` — a short ONE-LINE step-by-step walkthrough of how it works in practice ("1) the user … 2) the form …"), not the acceptance-criteria list re-typed. Where you resolved a trigger the engine could not trace (typically a helper invoked from another method's body), add \`trigger\` and \`from\`. For a row whose behaviour is defined outside your scope — a \`mixin:\` member or the method wiring one in, an externally-assigned method, a \`message:\` counterpart in another schema, a module dependency — ALSO name the body's own card as \`bodyCard\`/\`bodyAc\` (usually a shared-core card from the list above): the criteria that gate the behaviour live there, not in the wiring card.
