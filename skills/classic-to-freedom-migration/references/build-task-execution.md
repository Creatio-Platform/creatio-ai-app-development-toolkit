...
**Build preflight (Contract rule 7), scoped to YOUR task.** Before you create or edit the artifact your task names: (a) the plan's `⚠ Confirm` list is your worklist — every item is RESOLVED by running its on-stand query and recording the answer (DCM `SysSchema ManagerName='DcmSchemaManager'`, `ProcessInModules`, `SysModuleReport`, `get-component-info`), not deferred as "probably N/A"; (b) you build the plan's layout/components exactly — every island, tab, group, and both halves of a two-part component. Any simplification is a proposal to the user, not a silent change; (c) you bind every element to the COLUMN it shows, and name it for that column. → the mapping reference's build recipes.

**(c) in full — element naming and binding.** The element for the `Contact` column is named `Contact` or `ContactField`, and it is bound `$PDS_Contact`. A business rule targets that same element, and a list column keeps its `PDS_<Column>` code. That is the identity `--verify` accepts a `fields`, `rule` or `listcolumns` row on, and it is restated on those rows in the `Closed by` cell of your task file.

> The **binding** is what the gate resolves, and the name is what keeps the built page legible against the plan. Both matter, for different reasons. An element whose name drops its column — `RoleInCompanyField` for `Job`, `RequestField` for `InternalRequest` — is matched on its binding alone; it passes, but nobody reading the page can tell which plan row it satisfies. Name it for the column and the two agree.
>
> Why this gets a clause of its own: two runs of the same section built the page correctly, read `0/19 expected fields present` and `2/5 business rule(s) matched`, and each spent a repair round renaming elements that had never been wrong. **Do NOT rename elements on an already-built page to satisfy the gate.** If a row is open and the element is bound to the right column, that is a finding for `## Notes`, not work.
