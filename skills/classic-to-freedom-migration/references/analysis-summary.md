...
- **Every fact lives in ONE place — never restate it.** scope → the `Scope` line; owning/target package +
  ...
  on-stand decisions + risks → `⚠ Confirm`; a member declared here but defined elsewhere → `⚠ Imperative
  members`. If a line repeats something already shown above, cut it.
...
page, then form page (Layout → Logic → ⚠ Custom methods → ⚠ Other declared logic → ⚠ Confirm), then child pages under `Child page mappings`. Then
