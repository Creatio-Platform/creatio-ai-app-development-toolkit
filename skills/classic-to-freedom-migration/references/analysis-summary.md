...
2. **The per-page design spec** — generated by `node engine/migrate.mjs <manifest> --spec` and presented
   **verbatim** (see `references/page-design-spec.md`): one `Layout` table (structure + contents), a
   `Business rules` table (behaviour), the `⚠ Custom methods` / `⚠ Other declared logic` worklists, and the
   `⚠ Confirm before I build` worklist. This is the "what goes where".
...
- **Every fact lives in ONE place — never restate it.** scope → the `Scope` line; owning/target package +
  ...
  placement, type, rule → the design-spec `Layout` table; behaviour (filters/handlers/process) → `Business rules`;
  on-stand decisions + risks → `⚠ Confirm`; a member declared here but defined elsewhere → `⚠ Other declared
  logic`. If a line repeats something already shown above, cut it.
...
page, then form page (Layout → Business rules → ⚠ Custom methods → ⚠ Other declared logic → ⚠ Confirm), then child pages under `Child page mappings`. Then
...
Then the generated design spec (`Layout` + `Business rules` + `⚠ Custom methods` + `⚠ Other declared logic` +
`⚠ Confirm`) for `Applicant1Page` follows.
