...
2. **The per-page design spec** — generated by `node engine/migrate.mjs <manifest> --spec` and presented
   **verbatim** (see `references/page-design-spec.md`): one `Layout` table (structure + contents), a
   `Logic` table (behaviour), the `⚠ Imperative logic` / `⚠ Imperative members` worklists, and the
...
page, then form page (Layout → Logic → ⚠ Imperative logic → ⚠ Imperative members → ⚠ Confirm), then child pages under `Child page mappings`. Then
...
Then the generated design spec (`Layout` + `Logic` + `⚠ Imperative logic` + `⚠ Imperative members` +
