...
# Bumped whenever the counting rule changes (ENG-95856: usage deduplicated by
# message.id). Carried in `summary()` and printed by the CLI so two reports
# generated by different code -- e.g. a number pasted into a Jira comment
# before this fix, compared against a fresh run after it -- are never read as
# comparable without the reader noticing the version differs.
COUNTER_VERSION = 2
