...
function profileCardTarget(c, entity, info) {
  ...
  if (native) {
    ...
    return {
      ...
      reason: `embedded profile card '${c.key}' (classic '${c.schemaName || "?"}', profiled entity '${entity}') → Freedom '${native.type}' in the SIDE PROFILE: insert it into the side-profile container (e.g. 'SideAreaProfileFieldFlexContainer') with 'referenceColumn': '$${c.masterColumnName}' and 'readonly': true — the master lookup '${c.masterColumnName}' holds the profiled record's Id, and the component loads the record itself (no extra data source).${pkgNote} It renders ${native.shows}.${colsNote}${flagsNote} Any classic value the component does not render must be added BESIDE the card as read-only fields over a lookup-path data-source attribute: merge into the PDS attributes a '{ path: "${c.masterColumnName}.<column>", type: "ForwardReference" }' entry per value and bind a read-only field to it.${backRef}${baseNote}`,
    };
  }
  const entityNote = entity ? `profiled entity '${entity}'` : "profiled entity NOT resolved";
  return {
    freedom: null, package: null, fields: cols,
    reason: `embedded profile card '${c.key}' (classic '${c.schemaName || "?"}', ${entityNote}) has NO native Freedom compact-profile component — rebuild it as its OWN 'crt.GridContainer' island in 'SideAreaProfileContainer' (single-column, styled like the island the template already provides) holding READ-ONLY fields of the linked record: per shown column merge a lookup-path attribute into the PDS attributes — '{ path: "${c.masterColumnName}.<column>", type: "ForwardReference" }' — and bind a read-only field to it, plus the record link (the classic header was a hyperlink opening the profiled record).${colsNote}${flagsNote}${backRef}${baseNote} Do NOT drop the card and do NOT collapse it into the master's own fields.`,
  };
}
