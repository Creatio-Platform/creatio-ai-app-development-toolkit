...
- **`--start` enforces the queue, not just the ledger.** It refuses a task whose `dependsOn` has not closed, and
  ...
  signature, and every other check passes. Tasks writing DIFFERENT artifacts may be open at once, so the rule
  compares `writesTo` rather than counting clocks, and a read-only task never conflicts.
