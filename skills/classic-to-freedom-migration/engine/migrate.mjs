...
//
// ENG-98554 — `viewModelConfig` is OPTIONAL, exactly like the `modelConfig` beside it, and that is a contract
// decision rather than an oversight: requiring it would invalidate every payload already recorded — the ENG-98487
// artifact this change is proven against carries no `viewModelConfig` at all — and would have left the very runs
// that exposed the defect unable to demonstrate the fix. Supplied, it is the AUTHORITATIVE way to resolve a
// component's attribute binding to the column it reads; absent, the gate strips the `$` / `PDS_` prefix instead.
const BUILT_SHAPE = '{ "pages": { "main": { "viewConfig": <get-page bundle.viewConfig>, "packageName": "…", "parentSchemaName": "…", "modelConfig": <get-page bundle.modelConfig — OPTIONAL, and the only way the gate can see a page that has no primary data source and therefore hangs the browser>, "viewModelConfig": <get-page bundle.viewModelConfig — OPTIONAL: maps the attribute a component is bound to ($PDS_Contact) onto the column it reads (PDS.Contact), which is how a field named for its widget rather than for its column is still matched to the right column. Without it the gate strips the $/PDS_ prefix, which is correct for a conventionally built page but cannot see an attribute named nothing like its column>, "businessRules": <read-page-business-rules result: { count, rules } — the page\'s persisted BusinessRule_* schemas, NOT a page-body grep> }, "list": { "viewConfig": <the LIST page, same shape>, "schemaUId": "…" }, "child:<Entity>": false }, "reachability": { "sectionRegistered": { "workplaces": <n counted on the stand>, "names": [...] } — a COUNT, not a flag: a workplace registration only ADDS, so the row closes at exactly 1, "miniPageWired": true, … }, "evidence": { "<id>": {…} }, "judge": { "<id>": { "convincing": true } } }';
