...
const WORKPLACE_HOWTO = "Resolve the section's `SysModule.Id` FIRST (nothing else can be filtered without it):"
  ...
  + " Classic module and the Freedom one — and they sit in DIFFERENT workplaces, so pick by module schema and"
  + " never by row order: resolve each `SectionModuleSchemaUId` via `odata-read SysSchema`"
  ...
  + " (measured: an ESQ select of `SysWorkplaceId` comes back in `notFoundColumns` while `SysWorkplace.Name`"
  + " resolves). Name each workplace via `odata-read SysWorkplace`"
