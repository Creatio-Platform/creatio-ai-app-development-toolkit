...
function renderListBuildNotes(lcs) {
  ...
  if (lcs.quickFilterConfigCompletedByBuilder) {
    L.push("", "> **Build note — a quick filter here is placement, not the finished component:** the plan carries its name, container, index, filtered column and control. The full quick-filter also needs its own filter config and value binding, so complete it from the component's documentation rather than from this placement alone.");
  }
  return L;
}
...
function renderSpecHeader(result, opts, entity, fields, cs) {
  ...
  if (!opts.embedded) {
    ...
    L.push(
      ...
      "> paraphrase**. Layout = structure + contents (one table). Business rules / Custom methods = behaviour. ⚠ = confirm before build.",
      "",
...
// ENG-96327: each note says WHAT the kind IS (the human-facing definition); the Freedom "rebuild/implement as…"
// prescriptions were dropped — that build guidance lives in the SKILL / build-executor references, which the
// build agent is handed. The approver only needs to recognise the item.
const MEMBER_KIND_NOTE = {
  mixin: "**mixin** — members defined in ANOTHER schema, so none of the behaviour is in this page body.",
  message: "**message** — sandbox wiring whose counterpart lives in another schema. A subscribe with no publisher found is an unresolved thread, not \"no behaviour\".",
  "attribute-virtual": "**attribute-virtual** — page UI state with NO entity column behind it, so no field insert carries it.",
  "attribute-imperative": "**attribute-imperative** — a sub-key defined as a FUNCTION: a computed value the engine reads as present but cannot evaluate.",
  "attribute-lookup-filter": "**attribute-lookup-filter** — the lookup is filtered in code (imperatively), not as a declarative FILTRATION rule.",
  "module-dep": "**module-dep** — `define()` dependencies with no row of their own: constants/enum modules hold the lookup values the rules compare against, utility modules hold logic the page calls.",
  "referenced-module": "**referenced-module** — renders UI OUTSIDE this page, so its controls are not part of this page's own changes.",
};
