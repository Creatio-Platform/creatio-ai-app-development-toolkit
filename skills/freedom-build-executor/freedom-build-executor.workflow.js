...
- HOW YOU REACH clio — THE SHELL CLI IS THE DEFAULT FOR THE HEAVY STAND READS. This is a NARROW, MEASURED EXCEPTION to the repository's MCP-first rule, written down as a scoped carve-out in \`AGENTS.md\` ("Tool surface preference" → "Scoped exception: freedom-build-executor heavy stand reads (ENG-95262)") and limited to the read commands named below and to this skill: over one build run \`get-page\` through MCP \`clio-run\` averaged 90 s with 11 timeouts while the SAME reads through the shell \`clio\` averaged 2.3 s with none, and one \`get-schema\` through \`clio-run\` hung for 1800 s (tracked as ENG-95262). Everything not named below stays MCP-first.
...
- NEVER BUILD YOUR OWN TRANSPORT TO clio. Do not write an MCP client, do not spawn \`clio mcp-server\` yourself, do not hand-craft JSON-RPC envelopes. A real run did this to escape MCP timeouts and paid ~300 s per call (a fresh server per invocation) while guessing envelope shapes for 20 minutes. The only two ways in are the MCP tools this session already exposes and the shell \`clio\` binary.
- A TIMED-OUT CALL MEANS SWITCH TRANSPORT, NOT RETRY. \`error-class=creatio-timeout\`, \`timed out after 120s\`, or \`sent no response or progress\` is a fault of the path, not of the stand: re-issuing the same call over the same transport reproduces it. One real run timed out on \`get-page\` for a SINGLE page seven times across two agents — about 14 minutes for a read the CLI answered in 2 s. **\`sent no response or progress\` and \`error-class=creatio-timeout\` get NO retry — switch transport on the first occurrence.** \`sent no response or progress\` is the 1800 s wedge, so a "retry once" there costs half an hour on a path this same sentence calls deterministic. A single retry is allowed only for the cheap \`timed out after 120s\` class, and the retry-once idiom otherwise belongs to the wake-up row in \`./references/03-failure-and-park-policy.md\`. That file classifies all of these as environment faults, which never spend a unit's round budget.
