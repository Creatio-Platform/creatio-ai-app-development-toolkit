...
function workplaceBindingsLine(wb, wrap) {
  ...
  return `sectionRegistered's OWN counted workplace bindings THIS ROUND: ${wb.count}${names.length ? ` (${names.join(', ')})` : ''} — carry this into \`reachability.sectionRegistered\` unless your own on-stand count disagrees, in which case YOUR count wins (say so in \`notes\`).`;
}
...
2. \`reachability\` — for each applicable key, \`true\` ONLY after you confirmed the wiring on-stand, \`false\` when you confirmed it is absent, and OMIT the key when you did not check. Return what you wrote in \`reachabilityWritten\` as the strings 'true' / 'false' / 'unset'.
   ...
   - **CARRY THE BUILD UNIT'S OWN COUNT FORWARD (ENG-95470 / defect 4).** If the \`sectionRegistered\` unit ran this round, the claims block above (WHAT THE BUILD AGENTS CLAIMED) carries its OWN counted \`workplaceBindings\` line — write THAT count into \`reachability.sectionRegistered\` even when you could not (or did not get to) independently re-derive the count yourself this round: a run where ONLY your own on-stand check counted left the row at \`reachability: {}\` forever whenever that check was skipped or missed, despite the section being genuinely registered. If you DID independently count and the two disagree, YOUR count wins and say so in \`notes\` (the claim is the build unit's report, not a second ground truth).
...
while (true) {
  ...
  for (const id of lastVerifier?.evidenceWritten || []) {
    ...
    if (earnedBeforeRound.has(id) && !builtThisRound.includes(owner)) {
