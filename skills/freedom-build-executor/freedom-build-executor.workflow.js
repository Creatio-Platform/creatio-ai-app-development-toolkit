...
- HOW YOU REACH clio — THE SHELL CLI IS THE DEFAULT FOR THE HEAVY STAND READS. This is a NARROW, MEASURED EXCEPTION to the repository's MCP-first rule (\`AGENTS.md\`), scoped to the read commands named below and to this skill: over one build run \`get-page\` through MCP \`clio-run\` averaged 90 s with 11 timeouts while the SAME reads through the shell \`clio\` averaged 2.3 s with none, and one \`get-schema\` through \`clio-run\` hung for 1800 s (tracked as ENG-95262). Everything not named below stays MCP-first.
  - **Shell CLI** (\`clio <command> -e ${q(input.environment)} …\`) for the heavy stand reads — \`get-page\`, \`list-pages\`, \`list-app-sections\`, \`get-schema\`, \`get-related-page-addon\`, and any SQL/OData read.
  - **MCP, unchanged, for everything else.** Resident tools (\`resident=true\` in the \`get-tool-contract\` index — \`get-guidance\`, \`get-tool-contract\` among them) are called NATIVELY by their own tool name; never wrap a resident tool in \`clio-run\`. Non-resident tools go through \`clio-run\` as always. Writes (\`create-page\`, \`update-page\`, \`create-app\`, \`create-app-section\`) stay on MCP unless MCP is the transport that is failing.
  - **PROBE ONCE, before you rely on the CLI:** \`clio --version\` and \`clio ping -e ${q(input.environment)}\`. If the binary is missing or that environment is not registered for the shell, use \`clio-run\` for everything and say so in \`notes\` — do NOT register environments or install anything to make the CLI work.
...
- A TIMED-OUT CALL MEANS SWITCH TRANSPORT, NOT RETRY. \`error-class=creatio-timeout\`, \`timed out after 120s\`, or \`sent no response or progress\` is a fault of the path, not of the stand: re-issuing the same call over the same transport reproduces it. One real run timed out on \`get-page\` for a SINGLE page seven times across two agents — about 14 minutes for a read the CLI answered in 2 s. Retry once at most, then move to the other transport. \`./references/03-failure-and-park-policy.md\` classifies these as environment faults, which never spend a unit's round budget.
...
// build agent to, so their `clio help` output is cached next to the contracts.
const REFS_CLI_HELP = ['get-page', 'list-pages', 'list-app-sections', 'get-schema', 'get-related-page-addon',
  'create-related-page-addon', 'update-page']
...
2b. \`${REFS_DIR}/cli-usage.md\` — the CLI half of the same knowledge, because stand reads default to the shell \`clio\`. Record \`clio --version\` and the result of \`clio ping -e ${q(input.environment)}\`, then \`clio help <command>\` VERBATIM for each of: ${REFS_CLI_HELP.join(', ')}. If the shell \`clio\` is missing, or that environment is not registered for it, write that fact as the whole file and put it in \`notes\` — every build agent then knows to stay on \`clio-run\` instead of rediscovering it one timeout at a time. Do not register environments and do not install anything to make the CLI work.
