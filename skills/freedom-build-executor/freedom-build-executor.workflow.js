...
// run onto the Agent route, which is where the divergent state of A2 came from. The budget is three attempts,
// spaced by a bounded delay so a retry can land outside a short host-side rejection burst. Bounded and never
// silent: each attempt is logged, and exhausting them is still the honest `reconcile-failed` stop, not a run that
// proceeds on a state nobody produced.
const RECONCILE_ATTEMPTS = 3
// Overridable (args.reconcileRetryDelayMs) so the golden suite runs the retry paths without sleeping.
const RECONCILE_RETRY_DELAY_MS = Number.isFinite(args?.reconcileRetryDelayMs) ? args.reconcileRetryDelayMs : 5000
async function reconcileAgent(roundNo, label) {
  for (let attempt = 1; attempt <= RECONCILE_ATTEMPTS; attempt += 1) {
    ...
    if (attempt < RECONCILE_ATTEMPTS) {
      log(`Reconcile (${label}) returned nothing on attempt ${attempt} of ${RECONCILE_ATTEMPTS} — retrying the SAME call; a rejection here has been transient before, and switching routes over it is what split the state file`)
