...
WORKFLOW_META_BLOCK_OPENER = "export const meta"
...
def workflow_meta_name(script_path: Path) -> str:
    ...
    match = WORKFLOW_META_NAME_PATTERN.search(meta_block(text, script_path))
    if not match:
        raise RuntimeError(
    ...
    name = match.group(1)
    # Validated HERE, before the value becomes a path component. The script is machine-generated by
    # inlining `_workflow-core/` modules and prompt text, and `installer/update.py` re-runs this
    # provisioner over `~/.claude/plugins/cache/`, i.e. over content that arrives from a marketplace
    # update rather than from a reviewed checkout — unattended, on every plugin update.
    if not WORKFLOW_META_NAME_ALLOWED.fullmatch(name) or Path(name).name != name:
        raise RuntimeError(
            f"Workflow `meta.name` is not usable as a filename: {name!r} in {script_path}. "
            f"A provisioned name must match creatio-[A-Za-z0-9._-]+ and contain no path separator, "
            f"because it is written to ~/.claude/workflows/<name>.js."
        )
    return name


def meta_block(text: str, script_path: Path) -> str:
    """Return the text of the script's ``export const meta = {...}`` object literal.

    The name is read out of THIS slice rather than out of the whole file. The pattern is
    line-anchored but not block-anchored, so the first line-initial ``name:`` anywhere in the file
    used to win — and these scripts inline core modules and prompt text, so a future line beginning
    ``name: '...'`` would silently supply the destination filename.
    """
    opener = text.find(WORKFLOW_META_BLOCK_OPENER)
    if opener < 0:
        raise RuntimeError(
            f"Workflow script declares no `{WORKFLOW_META_BLOCK_OPENER}` block, so its `meta.name` "
            f"cannot be located: {script_path}"
        )
