...
- **Drift risk:** if clio rewords that note, FAULT 3 silently becomes a no-op and the gate degrades to a bare model attestation, with every in-repo test still green (the fault tests feed author-written strings that carry the tokens by construction). A **token-drift test** in `engine-tests/classic-to-freedom/` pins the exact tokens so a wording change fails a named test rather than switching the cross-check off unseen.
