...
- If the current request provides a Creatio URL that matches no registered environment and no credentials were given, decide by the URL host against a **known Creatio host pattern** — the closed set: a Creatio cloud host (`*.creatio.com`), an internal Creatio development host (`*.tscrm.com`, or a single-label `ts1-*` host with no dots such as `ts1-core-dev04` — a dotted host like `ts1-evil.attacker.com` does NOT match), or `localhost` / `127.0.0.1`. This is a closed list, not a broad category; extend it explicitly if more patterns are ever needed.
