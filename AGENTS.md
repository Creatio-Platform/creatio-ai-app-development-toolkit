...
Declared coupling: freedom-build-executor reads clio's `get-component-info` note (ENG-95468):

- The stand-provenance gate cross-checks an agent's `resolvedFrom: 'stand'` classification against **free text that clio owns** — the `get-component-info` probe-failure note, matched for the catalog-fallback tokens `probe-error` / `latest-fallback` (`componentSweepFaults` FAULT 3, `skills/_workflow-core/build-executor/helpers.mjs`). This is a **best-effort** cross-check, not a machine-verified contract: the tokens are prose, not a structured MCP field, so "Repository docs must not define an independent MCP API contract" is not violated — but the dependency on that prose is real and is declared here so it is not invisible from clio's side.
- **Drift risk, and what is and is NOT guarded:** if clio rewords that note, FAULT 3 silently becomes a no-op and the gate degrades to a bare model attestation. **No in-repo signal detects a clio-side wording change today** — every in-repo test stays green, because the fault tests feed author-written strings that carry the tokens by construction. The token-pin test in `engine-tests/classic-to-freedom/run-infra.mjs` guards a *different* failure: it pins the token list and this declaration against a silent **in-repo** edit (someone loosening or dropping either), so that guard does not quietly disappear. Closing the clio-side gap needs the fixture named in **Durable fix** below — a captured real probe-failure response — which is not yet in place.
- **Durable fix:** clio emitting a structured `resolvedFrom` (or echoing `resolvedFromReason` verbatim) across the MCP contract removes the free-text guess entirely; at that point the gate rests on the tool signal and this coupling note is deleted. Recorded in `skills/freedom-build-executor/references/05-decision-records.md` (DR-8). The residual not-yet-closed piece is a fixture captured from a real `get-component-info` probe-failure response with the clio version recorded beside it.
