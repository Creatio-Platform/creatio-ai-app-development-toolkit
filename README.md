...
10. Agent runs Agent 3 and returns technical planning artifacts.
11. When execution handoff is requested, Agent 4 generates `output/<AppName>/docs/execution-runbook.md`.
12. Internal gate tokens, old workflow-state details, and scripts stay hidden from developer-facing dialogue unless they are real blockers.
...
5. Execution handoff is developer-triggered: Agent 4 converts `plan.md` into `output/<AppName>/docs/execution-runbook.md` for direct Developer + AI execution through `clio` MCP.
6. Existing-app branching remains explicit in the workflow, and the canonical discover/inspect/mutate path and fallback tool guidance are owned by `clio` rather than this repository.
   Resolve these flows through `docs://mcp/guides/app-modeling` and `docs://mcp/guides/existing-app-maintenance`.
7. This orchestrator workflow does not execute runtime `clio` MCP mutation calls; it prepares handoff artifacts only.

All generated artifacts are under `output/<AppName>/`.

## Runtime Scripts (Optional Reference)

These scripts remain available as optional helper tooling for Developer + AI execution flows.
They are not a required critical path for Agent 4 completion.
...
- Unix/macOS wrappers stay supported:
  ...
  - `bash scripts/check-execution-handoff.sh <AppName>`
- PowerShell peers are available on Windows:
  ...
  - `.\scripts\check-execution-handoff.ps1 <AppName>`
...
|-- Agent 4: Execution Handoff           -> docs/execution-runbook.md
|   `-- Developer + AI execute runbook directly through clio MCP
```
