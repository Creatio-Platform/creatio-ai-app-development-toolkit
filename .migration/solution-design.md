...
- **Ф2 Merge-рушій (pure):** + обгортка `get-effective-classic-schema`. Тест офлайн на фікстурах. ✅ **прототип готовий, golden 46/46** ([engine/](../skills/classic-to-freedom-migration/engine/)): SupportUnit + Contract(9 шарів) зливаються детерміновано, з provenance; те, що LLM рахував ~142k токенів, код робить миттєво. **F1** (порядок за HierarchyLevel + `warnings`) і **F2** (`seedLayers` + `unresolvedParents`) закриті.
- **Ф3 Mapper (pure):** ✅ **прототип готовий, golden 67/67** ([engine/mapper.mjs](../skills/classic-to-freedom-migration/engine/mapper.mjs)): **F3** — повне дерево вкладок/контейнерів (маршрут поля підйомом по предках; вкладка+grid емітяться раз і лише коли тримають поле); **F9** — payload лише зі schema-шарів (за походженням insert), base-template = layout-контекст. **Межа non-BaseModulePageV2** (звірка Case): нестандартний UI (SLA-таймер-мікровіджети, референсні AMD-модулі, кастомні кнопки, `hint`-тултіпи) — не мапиться 1:1, але **гучно** виноситься в `needsDecision` (`unmapped-component`/`referenced-module`/`field-hint`), не зникає тихо. Лишаються reconcile-diff (A3/F4) → 🎯 демо-віха: показати ефективну Classic + чернетку Freedom **без запису**.
