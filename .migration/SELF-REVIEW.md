...
## `--plan` (весь скелет генерується) + рекурсія дочірніх сторінок

Сьомий прогін (успіх на новому коді) виявив рецидив **презентації**: движок згенерував УСЕ (Layout з реальними назвами, 2 острови, Section-блок з list-columns/add-record, process-флаг, Follow-on), а агент при показі **відкурував** — викинув `### Section`, переписав Layout, зробив власний «Structure analysis» замість Overview/Pages. Тобто не «стейл-код», а агент не вставляє дослівно.

**Фікс — генерувати ВЕСЬ план, не лише spec (merge 49/49, mapper 126/126, +6):**
- `migrate.mjs --plan` (новий `renderPlan`) віддає повний скелет: **Overview / What it does / Pages з `<FILL: …>`-плейсхолдерами** (лише рішення агента: scope/env/package/approach/бізнес-речення/темплейти) + **згенерований design-spec** (Layout/Section/Logic/Confirm) + **Child pages** (рекурсія). Size пре-заповнено. Агент лише підставляє FILL і **вставляє дослівно**; корекції — окремим `Adjustments`-списком, **не редагуючи/викидаючи** згенеровані секції.
- SKILL: домінантна gate-інструкція (рядок 12) + output-rules + крок 6 → тепер `--plan` verbatim (не `--spec`, не переказ). Крок 7 → **рекурсивна міграція деревом**: на кожен рядок Child pages запустити pipeline для дочірньої edit-сторінки (+ add-record mini page).
- **Details «повністю»** (обрано Full-recursive): рушій на кожну кастом-деталь віддає child entity + editPage (з `getEditPageName`, інакше «resolve via list-pages») + editable (view-only якщо `getAddRecordButtonVisible→false`) → `result.childPages` + Child-pages таблиця. Самі дочірні сторінки будуються рекурсивно (крок 7).
...
**Зауваження юзера #2:** у `Layout` сортувати — спочатку весь профіль, тоді таби. Зроблено: `regionRank` (Side profile/Header=0 → Tabs=1 → Header/top widgets=2 → Card actions=3 → ⚠fallback=4), stable за first-seen; острови більше не перемежовуються з табами. +golden #6.

**PR-гігієна (B+A): тест-харнес зі скіла + санітизація фікстур.** Плагін ставиться повним клоном репо → фікстури/раннери потрапляли клієнту, а реальні стенд-експорти давали Sonar-дублювання 15.1%.
- **B:** `run.mjs`/`run-mapper.mjs`/`_testkit.mjs`/`fixtures/` винесено з `skills/.../engine/` у кореневий `engine-tests/classic-to-freedom/` (раннери імпортують рушій відносним шляхом; `ENGINE_DIR` для migrate.mjs CLI). Скіл тепер має лише рантайм (4×`.mjs`+package.json/lock/README). CI `pr.yml` working-directory оновлено.
- **A (компактна синтетика):** SupportUnit-фікстури переписано вручну (прибрано verbatim module-дампи з `recordId` — головний реальний експорт + дубль 44.8%+91.9%); `employeescore` видалено (мертвий, дубль 36.4%).
- **A (light-touch для contract):** знахідка — contract-файли це чиста СТРУКТУРА (не дані, майже без GUID) і найцінніший golden (9 реальних шарів). Тому НЕ переписувала: `WorkContractsProcess` обрізано до 6 asserted-оп-ів (verbatim), `WorkSalesBase` спорожнено (non-asserted filler — доведено емпірично). Решта 7 шарів лишились реальними. Усі 5 дубль-джерел прибрано.
- Оракул — голдени (merge 49/49, mapper 138/138 на кожному кроці). Sonar-rating-и вже 1/1/1; дублювання мало впасти до ~0.

**Зауваження юзера #3 (питання «чому не показуємо, який процес запускати?»):** пояснив межу *схема vs дані* — базова `ProcessButton` («run process by record») жодного процесу не називає, а конкретний процес у меню (напр. «Recruiting. Security Check») підключений до entity **даними** (`SysProcessEntityConnection`/`VwSysProcessEntityConnection`, властивість процесу «в якому розділі показувати»), а не тілом схеми; тому рушій (парсить тіла шарів) його не бачить і за E1 **не вигадує** ім'я. Рушій називає процес ЛИШЕ якщо метод викликає `executeProcess`/`RunProcessRequest` з **літералом** (`processLaunch.names`). Фікс = зробив прапорець **дієвим**: (a) SKILL — новий абзац «Name the process behind a Run-process action»: коли бачиш флаг, зроби **discovery на стенді** (`execute-esq`/`odata-read` по `VwSysProcessEntityConnection` фільтром за entity) і назви підключені процеси в плані (resolve флага / Adjustments), відділяючи generic-кнопку від конкретного процесу; (b) `designspec.mjs` — card-action та Logic-рядки тепер підказують ЯК резолвити («resolve via connected processes on-stand (VwSysProcessEntityConnection)»), а коли ім'я вже відоме — «verify process name/binding». Межа лишається: discovery оркеструє агент (рушій даних не фетчить).

**Зауваження юзера #4 (на скріні старої сесії): «нам же і мапінг цих сторінок потрібен».** Child-сторінки лише ПЕРЕЛІЧувались (Pages рядки `Rebuild (child)` / стара таблиця), але самого мапінгу (Layout/Logic по кожній) не було — він вивалювався на агента й міг не з'явитись. Фікс = **справжня рекурсія в рушії** (merge 49/49, mapper 136/136, +3):
- `migrate.mjs` приймає **`childPageSchemas`** — вкладений маніфест на кожну дочірню edit-сторінку (keyed за editPage / child-entity / entity+"Page"). `runMigration` **рекурсивно мапить** кожну (depth-cap 2 проти циклів) і кладе її `designSpec` у `childPages[].spec`.
- `designspec.renderPlan` → нова секція **`### Child page mappings`**: для дочірньої зі схемою — її повний design-spec, вкладений (заголовки демоутнуті `demoteHeadings`: `##`→`####`, `###`→`#####`); без схеми — явний слот **`<FILL: recursive sub-migration>`** (мапінг лишається ОБОВ'ЯЗКОВИМ, а не рядком, який агент вважає готовим). Помилка парсингу child-маніфесту → нота, рядок лишається.
...
**Зауваження юзера #6: перейменувати `Pages` → `Main scope` + розкривати в порядку таблиці.** «Pages» — це індекс основних сторінок, які далі розкриваються; далі має йти в тій же черговості: спершу List page (описати), потім form page (Layout/Logic). Раніше List page (блок Section) стояв ПІСЛЯ Layout форми. Фікс (merge 49/49, mapper 138/138):
- `designspec.mjs`: `### Pages` → **`### Main scope`** (з поміткою, що це індекс); design-spec переструктуровано — заголовок `## Design spec — <entity> (generated)` (без «form page»), потім **`### List page`** ПЕРШИМ (був `### Section (list page)`), потім **`### <entity> form page`** з **`#### Layout` / `#### Logic` / `#### ⚠ Confirm`** (демоут на рівень). Child page mappings і далі в кінці. Порядок читання = порядок рядків Main scope: list → form → child. Child-специ демоутяться на +2 (тепер `###### Layout`).
- Голдени: `### Layout`→`#### Layout`, `### Logic`→`#### Logic`, `### Confirm`→`#### Confirm`, `### Section (list page)`→`### List page` (+ перевірка «List page перед form page»), `### Pages`→`### Main scope`, child `##### Layout`→`###### Layout`.
- Доки: `page-design-spec.md` (шаблон+приклад під List page/form-page/`####`), `analysis-summary.md` (`### Pages`→`### Main scope`), SKILL (усі згадки Pages/`### Section (list page)` → Main scope / List page; verbatim-нота забороняє ще й **reorder**).
