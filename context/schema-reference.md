...
- The raw BaseEntity inherited column list above does not include template-generated columns. The runtime application context may include section entities that already contain `Name` from the app template; always inspect the refreshed context rather than assuming which columns exist.
