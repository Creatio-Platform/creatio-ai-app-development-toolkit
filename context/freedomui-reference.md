# Freedom UI Reference

## Page JS Structure (AMD Module)

Every Freedom UI page is a JavaScript AMD module:

```javascript
define("UsrTodoTask_ListPage", /**SCHEMA_DEPS*/[]/**SCHEMA_DEPS*/, function/**SCHEMA_ARGS*/()/**SCHEMA_ARGS*/ {
	return {
		viewConfigDiff: /**SCHEMA_VIEW_CONFIG_DIFF*/[
			// UI layout and controls
		]/**SCHEMA_VIEW_CONFIG_DIFF*/,
		viewModelConfigDiff: /**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/[
			// Attribute bindings to data source
		]/**SCHEMA_VIEW_MODEL_CONFIG_DIFF*/,
		modelConfigDiff: /**SCHEMA_MODEL_CONFIG_DIFF*/[
			// Data source configuration
		]/**SCHEMA_MODEL_CONFIG_DIFF*/,
		handlers: /**SCHEMA_HANDLERS*/[]/**SCHEMA_HANDLERS*/,
		converters: /**SCHEMA_CONVERTERS*/{}/**SCHEMA_CONVERTERS*/,
		validators: /**SCHEMA_VALIDATORS*/{}/**SCHEMA_VALIDATORS*/
	};
});
```

## viewConfigDiff Operations

The `viewConfigDiff` array contains operations that modify the parent page template:

### Operation Types

| Operation | Description |
|-----------|-------------|
| `merge` | Modify existing element properties |
| `insert` | Add new element |
| `remove` | Remove existing element |
| `move` | Move element to different parent |

### Insert Operation

```json
{
	"operation": "insert",
	"name": "UsrTitle",
	"values": {
		"layoutConfig": {
			"column": 1,
			"colSpan": 1,
			"row": 1,
			"rowSpan": 1
		},
		"type": "crt.Input",
		"label": "$Resources.Strings.PDS_UsrTitle_label",
		"control": "$PDS_UsrTitle",
		"visible": true,
		"readonly": false,
		"placeholder": "",
		"tooltip": ""
	},
	"parentName": "GeneralInfoTab",
	"propertyName": "items",
	"index": 0
}
```

### Merge Operation

```json
{
	"operation": "merge",
	"name": "PageTitle",
	"values": {
		"caption": "#MacrosTemplateString(#ResourceString(PageTitle_caption)#)#",
		"visible": true
	}
}
```

### Remove Operation

```json
{
	"operation": "remove",
	"name": "DataImportButton"
}
```

## Control Types

| DataValueType | Control Type | Type String |
|---------------|-------------|-------------|
| ShortText, MediumText, LongText | Input | `crt.Input` |
| MaxSizeText, RichText | RichTextEditor | `crt.RichTextEditor` |
| Integer, Float, Money | NumberInput | `crt.NumberInput` |
| Boolean | Checkbox | `crt.Checkbox` |
| DateTime, Date, Time | DateTimePicker | `crt.DateTimePicker` |
| Lookup | ComboBox | `crt.ComboBox` |
| Image, ImageLookup | ImageInput | `crt.ImageInput` |
| Color | ColorPicker | `crt.ColorPicker` |

## List Page Template

### DataTable Column Configuration

In list pages, configure visible columns in the DataTable:

```json
{
	"operation": "insert",
	"name": "DataTable",
	"values": {
		"type": "crt.DataGrid",
		"features": {
			"rows": {
				"selection": {
					"enable": true,
					"multiple": true
				}
			}
		},
		"items": "$DataTable_Items",
		"primaryColumnName": "PDS_Id",
		"columns": [
			{
				"id": "f252f582-...",
				"code": "PDS_UsrTitle",
				"path": "UsrTitle",
				"caption": "#ResourceString(PDS_UsrTitle)#",
				"dataValueType": 1
			},
			{
				"id": "a7b2c3d4-...",
				"code": "PDS_UsrStatus",
				"path": "UsrStatus",
				"caption": "#ResourceString(PDS_UsrStatus)#",
				"dataValueType": 10,
				"referenceSchemaName": "UsrTodoTaskStatus"
			},
			{
				"id": "e5f6a7b8-...",
				"code": "PDS_UsrDueDate",
				"path": "UsrDueDate",
				"caption": "#ResourceString(PDS_UsrDueDate)#",
				"dataValueType": 8
			}
		],
		"visible": true
	},
	"parentName": "DataTableContainer",
	"propertyName": "items",
	"index": 0
}
```

**Column fields:**
- `id` — unique GUID for this column definition
- `code` — `PDS_<ColumnName>` (data source prefix + column name)
- `path` — actual entity column name
- `caption` — localized caption (uses `#ResourceString()#`)
- `dataValueType` — numeric type ID (see entity-types.md)
- `referenceSchemaName` — only for Lookup columns

### List Page Add Button Configuration

```json
{
	"operation": "merge",
	"name": "AddButton",
	"values": {
		"clicked": {
			"request": "crt.CreateRecordRequest",
			"params": {
				"entityName": "UsrTodoTask"
			}
		}
	}
}
```

## viewModelConfigDiff

Binds page attributes to the data source:

```json
[
	{
		"operation": "merge",
		"path": [
			"attributes"
		],
		"values": {
			"PDS_UsrTitle": {
				"modelConfig": {
					"path": "PDS.UsrTitle"
				}
			},
			"PDS_UsrStatus": {
				"modelConfig": {
					"path": "PDS.UsrStatus"
				}
			},
			"PDS_UsrDueDate": {
				"modelConfig": {
					"path": "PDS.UsrDueDate"
				}
			}
		}
	}
]
```

## modelConfigDiff

Configures the primary data source:

### List Page

```json
[
	{
		"operation": "merge",
		"path": [
			"dataSources"
		],
		"values": {
			"PDS": {
				"type": "crt.EntityDataSource",
				"config": {
					"entitySchemaName": "UsrTodoTask"
				},
				"scope": "viewElement"
			}
		}
	}
]
```

### Form Page

```json
[
	{
		"operation": "merge",
		"path": [
			"dataSources"
		],
		"values": {
			"PDS": {
				"type": "crt.EntityDataSource",
				"config": {
					"entitySchemaName": "UsrTodoTask"
				}
			}
		}
	}
]
```

## Form Page Layout

### Field Layout in Grid

Fields are placed using `layoutConfig`:

```json
{
	"operation": "insert",
	"name": "UsrTitle",
	"values": {
		"layoutConfig": {
			"column": 1,
			"colSpan": 1,
			"row": 1,
			"rowSpan": 1
		},
		"type": "crt.Input",
		"label": "$Resources.Strings.PDS_UsrTitle_label",
		"control": "$PDS_UsrTitle",
		"visible": true
	},
	"parentName": "GeneralInfoTab",
	"propertyName": "items",
	"index": 0
}
```

- `column` — grid column (1-based)
- `colSpan` — how many columns to span
- `row` — grid row (1-based)
- `rowSpan` — how many rows to span

### ComboBox for Lookup Fields

```json
{
	"operation": "insert",
	"name": "UsrStatus",
	"values": {
		"layoutConfig": {
			"column": 1,
			"colSpan": 1,
			"row": 3,
			"rowSpan": 1
		},
		"type": "crt.ComboBox",
		"label": "$Resources.Strings.PDS_UsrStatus_label",
		"control": "$PDS_UsrStatus",
		"visible": true
	},
	"parentName": "GeneralInfoTab",
	"propertyName": "items",
	"index": 2
}
```

## Page Parent Templates

| Template | UId | Use Case |
|----------|-----|----------|
| ListPageV3Template | `b7b898d0-8c77-4953-c097-23fa6800da02` | Section list page |
| PageWithTabsFreedomTemplate | `3b2e117f-8c6b-4ca5-80a2-7ebb497cddf9` | Form page with tabs |
| PageWithRightAreaAndTabsFreedomTemplate | `5f8dd430-acf2-4e1a-80c8-77cf57e245ce` | Form with right area + tabs |
| LightFormPage | `ec5fd902-66ce-4139-a241-10ebd8addc40` | Light/mini form |
| MinimalCardTemplate | `0f8eb896-7b62-4dfa-bd55-a414f50932a7` | Minimal card |

## Handlers

Event handlers for page logic:

```javascript
handlers: /**SCHEMA_HANDLERS*/[
	{
		request: "crt.HandleViewModelInitRequest",
		handler: async (request, next) => {
			// Page initialization logic
			return next?.handle(request);
		}
	}
]/**SCHEMA_HANDLERS*/
```
