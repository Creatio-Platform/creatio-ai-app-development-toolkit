...
- `session_started`: first user input in the CAADT workflow. Emitted once per workflow — on a first-run grant the consent-persisting send is this emission (see the consent paragraph above).
...
- After consent is granted: emit `session_started` before discovery starts — the single first-run emission described above, not an additional send.
