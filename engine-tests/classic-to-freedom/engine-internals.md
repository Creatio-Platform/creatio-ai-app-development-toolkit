...
- Symbolic enums: `BusinessRuleModule` (RuleType/Property) and `ViewItemType` (only the confirmed 0/2/15) are **seeded** → they resolve. Other `ViewItemType` members and `Terrasoft.ContentType.*` are still symbolic → non-numeric (null); seed more of them against confirmed values (never guess — lesson E1).
