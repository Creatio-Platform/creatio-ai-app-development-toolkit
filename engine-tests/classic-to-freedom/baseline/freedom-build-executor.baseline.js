...
const RECONCILE_SHAPE = {
  ...
  verify: { kind: 'object', required: ['complete', 'missing', 'unverified', 'pending', 'pages'],
    types: { complete: 'boolean', missing: 'integer', unverified: 'integer', pending: 'integer', accepted: 'integer' },
...
function reopenKeySet(findingsPending, resolutionsPending, isExhausted, ...moreBudgeted) {
...
function completionLine(complete, { round, missing, unverified, parkedCount, unconsumedCount, pendingCount = 0 } = {}) {
  if (complete) return `COMPLETE after ${round} round(s): the engine gate is green`
  const buildDone = (missing ?? 0) === 0 && (unverified ?? 0) === 0 && !parkedCount && !unconsumedCount
  ...
  return `NOT COMPLETE after ${round} round(s): ${missing ?? '?'} MISSING + ${unverified ?? '?'} unconfirmed · ${parkedCount} parked unit(s) · ${unconsumedCount} unconsumed answer(s)`
}
