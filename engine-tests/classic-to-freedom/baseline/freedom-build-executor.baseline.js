...
const RECONCILE_SCHEMA = {
  ...
  properties: {
    ...
    componentResolution: {
      ...
      items: {
        ...
        properties: {
          ...
          // ENG-95468 (residual) — WHERE the answer came from: `'stand'` (this environment answered) or `'catalog'`
          // (it did not — `get-component-info` could not probe the environment and answered from its bundled
          // `latest` catalog instead). Only `'stand'` is a confirmation; a catalog answer STOPS the round rather
          // than clearing the gate on a round where nothing about the stand was checked. REQUIRED, because an
          // optional provenance field is a gate that is silently off on exactly the round that needs it.
    ...
    // The EMPTY prefix's wire form — hand-ported from the core: `{ schemaNamePrefix: null, schemaNamePrefixEmpty:
    // true }` decodes back to `''` on acceptance, so a bare `""` never rides this answer.
...
//
// HARD STOP 2's REPORT (ENG-95857). The engine publishes FOUR plan-level kinds and this is the whole vocabulary.
// Recognition is by CONTAINMENT and case-INSENSITIVE, not by reading the first two tokens: the entry is meant to
// be `--units.planGaps` copied verbatim, but the field is typed only as `string[]`, so a paraphrase or a pasted
// stderr line can arrive here — and a leading-token parse read `migrate.mjs: ⛔ GATE BLOCKED …` as the kind
...
// The kind list AS A LOG LABEL, with a word for the empty case: the join is empty exactly when nothing
// classified, so the one run whose classification failed printed a bare `[]` in both STOP lines (PR review).
function planGapKindLabel(planGaps) {
  return planGapKinds(planGaps).join(' · ') || 'unclassified'
}
...
function planGapNext(planGaps, tail = 'then re-run this build') {
  // Count the entries AS PUBLISHED (the STOP lines count the same array), quote only the ones carrying text, and
  // say so when that leaves nothing to quote. Then keep the engine's own entries on the CLASSIFIED path too: the
  // kind says which check fired, the remedy says where to go, only the entry text says what to fix (PR review).
...
// ENG-95930 (mode B) — the open rows are NO LONGER handed to the builder in this prompt; it reads them from its own
// scoped gate at the start of the round, and validates the slice is its own before repairing.
...
// gated here (the un-swept types are logged and the run continues: absence is not evidence). Silence would
// therefore reproduce the same unstopped round with a log line in place of the `resolved: true`. It takes a VALUE.
...
// stated value is the only thing that gates — an entry with NO `resolvedFrom` is left alone, so a state written
// before this field existed behaves exactly as it did. That drop path is closed where it belongs instead: the
// response-shape check makes `resolvedFrom` REQUIRED on every entry, so an agent answer missing it is refused and
// retried rather than quietly reaching this arithmetic without provenance.
...
// CASE-FOLDED (PR #159 review, Major 1): nothing upstream constrains this value, so an exact compare made `'Stand'`
// on a healthy round a terminal stop whose `next` sent the operator to fix DNS that was fine — a false positive
// worse than the defect the axis closes, and unrecoverable, since the stop offers no override. A genuinely unknown
// word still gates.
...
// One of two FIXED words reaches the operator text, never the agent's own string: `standUnconfirmedDetail` renders
...
// The resolutions this stand actually answered — everything except a STATED non-stand provenance (PR #159 review,
// Major 2). A catalog `resolved: false` is no more evidence about this stand than a catalog `resolved: true`, so it
// must never reach `componentTypeMismatches` and become a re-plan instruction; an entry with no provenance at all
// IS included, which keeps a plan/state predating the field behaving exactly as it did.
...
// clauses, shared by the package-precondition stop and the unvalidated-stand stop (PR #159 review, Major 2), because
// both read the SAME baseline Reconcile facts and a re-plan must see every axis whichever stop fired.
...
// The Reconcile answer's mechanical encoder, shipped INSIDE the prompt — hand-ported from the core so the rendered
// prompt stays byte-identical. No backslash, backtick or `${` appears in this source; the backslash it needs is
// built at runtime from char code 92, and the regex deliberately has no `u` flag (per-code-unit matching is what
// turns a surrogate pair into the two escapes JSON requires).
...
// One capture file PER DISPATCH — hand-ported from the core: the label distinguishes the call-sites and the
// workflow-level retries, and every retry is a fresh context restarting the in-prompt counter at 1.
...
It validates the raw file and writes an equivalent ASCII-only encoding — every non-ASCII character becomes a \\uXXXX escape, which parses back to the identical character, so every VERBATIM rule above still holds after decoding. If its parse fails, fix the RAW file and re-run it; never submit an answer the helper rejected. THE SIZE IT PRINTS IS THE WIRE SIZE, and it is your PRE-SUBMIT GATE: if it prints more than 16000 bytes, do NOT submit — the host's input cap would truncate the payload mid-flight and the whole attempt is lost. Shrink first, per the size rules above (counts, keys and ids in the answer; bulk stays on disk), re-compose, re-encode, and only then submit.
...
async function reconcileAgent(roundNo, label) {
  ...
  for (let attempt = 1; attempt <= RECONCILE_ATTEMPTS; attempt += 1) {
    ...
    // The prompt is composed ABOVE the try — hand-ported intent from the core: the only thing that can throw inside
    ...
    let answer
    try {
      answer = await agent(attemptPrompt, {
        agentType: 'general-purpose', schema: RECONCILE_SCHEMA, phase: 'Reconcile',
        label: attemptLabel,
      })
    } catch (e) {
      // The catch-all IS the narrow catch here: this reference calls `agent()` directly and the prompt is composed
...
// ENG-95857 — `state.planGaps` is `--units.planGaps` VERBATIM: the engine's own machine-readable classification
// of all FOUR plan-level checks, copied by Reconcile step 2 and topped up by nothing.
...
// FIRST on this stop point, ahead of the package stop and its queue-file re-read: when the component answers did
// not come from this stand, this round established nothing about the stand at all, so every verdict below it would
// be computed over a catalog. Stopping here also stops the agents the package stop would spend confirming a record
// on a round that is already over.
// NOT a re-plan, and the `next` says so: nothing here implicates the plan — a catalog `resolved: false` is no more
// evidence about this stand than a catalog `resolved: true` — so the move is to make the environment answerable and
// re-run. There is deliberately no override: a stand whose version cannot be probed while it is otherwise up
// produces the same catalog answer, and treating that as a confirmation is the defect, not a special case of it.
// SCOPED TO STAND-ANSWERED ENTRIES (PR #159 review, Major 2): a catalog `resolved: false` says nothing about this
// stand, so it must not become a re-plan instruction. On a healthy round every entry is stand-answered and this is
// the identity function, so the pre-existing behaviour is untouched.
...
// re-read, but AFTER the three axes above are computed (PR #159 review, Major 2) and after their coverage NOTEs:
// they are pure arithmetic, they cost no agent call, and this stop carries them, so a MIXED round — one catalog
// answer beside a stand-answered `resolved: false` — no longer hides a defect the stand actually confirmed behind a
...
if (standUnconfirmed.length) {
  ...
  return runReturn({
    ...
    staleQueueKeys: state.staleQueueKeys || [],
    newKeys: state.newKeys || [],
    next: [standUnvalidatedNext(standUnconfirmed, 'Nothing was built.'),
...
// WHY a unit was parked. ENG-95930 (mode B): the central verify is counts-only, so the reason carries the counts and
// a pointer to the on-disk table, never the open rows themselves.
...
// RETURNS whether it actually WROTE (ENG-96147 review, tetiana-moshon), and re-reporting the SAME route is not a
// write: the `sectionRegistered` unit reports its route again on every round it stays open.
...
// `mergeOrphanedPages` folds the orphan list, so a resumed run reports and re-persists the record instead of
// reading its own route as absent. FIRST RECORD WINS, same as the orphan merge.
...
async function buildRound(open) {
  ...
  for (const unit of open) {
    ...
    // Gated on THIS unit's own write, not on `standWrites.sectionRoute`: the latter stays set for the rest of the
    // run, so every later reach unit would have bought a persist agent for a write it never made.
    if (unit.kind === 'reach' && r.sectionRouteWritten) {
      r.sectionRouteWritten = false
      await persistPending('recording the section\'s navigation route')
    }
  }
...
if (DRY_RUN) {
  ...
  // open-row COUNT and points at `verify.md` for the rows themselves.
...
async function acceptReconciled(next, whereFrom) {
  ...
  // The three axes are computed HERE, above every mid-run stop, for the reason the baseline call site computes them
  // above its own (PR #159 review, Major 2): pure arithmetic, no agent call, and each stop below carries them, so a
